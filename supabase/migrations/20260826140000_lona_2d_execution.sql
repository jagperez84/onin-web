alter table public.production_work_sheet_line add column if not exists remainder_pieces jsonb not null default '[]'::jsonb;
...
begin
  ...
  for v_line in select * from public.production_work_sheet_line where work_sheet_id=p_work_sheet_id order by line_no loop
    ...
    v_rotated:=not(v_w>=v_cw and v_h>=v_ch);
    if v_rotated then v_cw:=(v_line.cut_dimension_values->>1)::numeric; v_ch:=(v_line.cut_dimension_values->>0)::numeric; end if;
    ...
    values(v_sheet.company_id,v_stock.warehouse_id,v_item.product_id,v_type.id,v_item.characteristic_id,1,now(),v_sheet.reference,'Consumo de lona 2D · '||v_sheet.code,jsonb_build_array(v_cw,v_ch)) returning id into v_movement_id;
    ...
    v_remnants:='[]'::jsonb;
    if v_w-v_cw>0 and v_ch>0 then v_remnants:=v_remnants||jsonb_build_array(jsonb_build_array(v_w-v_cw,v_ch)); end if;
    if v_h-v_ch>0 and v_w>0 then v_remnants:=v_remnants||jsonb_build_array(jsonb_build_array(v_w,v_h-v_ch)); end if;
    ...
    if v_recuttable then
      for v_cw,v_ch in select (x->>0)::numeric,(x->>1)::numeric from jsonb_array_elements(v_remnants) x loop
        if greatest(v_cw,v_ch)>=v_min and v_cw>0 and v_ch>0 then
          insert into public.warehouse_stock_item(warehouse_stock_id,product_id,characteristic_id,quantity,dimension_values,status,parent_stock_item_id,source_stock_movement_id,source_sales_order_id,source_sales_order_line_id,source_work_sheet_id,source_work_sheet_line_id,remnant_generated_at)
          values(v_item.warehouse_stock_id,v_item.product_id,v_item.characteristic_id,1,jsonb_build_array(v_cw,v_ch),'AVAILABLE',v_item.id,null,v_sheet.sales_order_id,v_sheet.sales_order_line_id,p_work_sheet_id,v_line.id,now()) returning id into v_new_item;
          ...
          insert into public.stock_movement(company_id,warehouse_id,product_id,movement_type_id,characteristic_id,quantity,movement_date,reference,notes,dimension_values)
          values(v_sheet.company_id,v_stock.warehouse_id,v_item.product_id,v_type.id,v_item.characteristic_id,1,now(),v_sheet.reference,'Remanente de lona · '||v_sheet.code,jsonb_build_array(v_cw,v_ch)) returning id into v_movement_id;
          ...
          insert into public.stock_movement(company_id,warehouse_id,product_id,movement_type_id,characteristic_id,quantity,movement_date,reference,notes,dimension_values)
          values(v_sheet.company_id,v_stock.warehouse_id,v_item.product_id,v_type.id,v_item.characteristic_id,1,now(),v_sheet.reference,'Merma de lona · '||v_sheet.code,jsonb_build_array(v_cw,v_ch));
        end if;
      end loop;
    end if;
  end loop;
