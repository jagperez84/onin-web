-- ONIN: al ejecutar una hoja de confección de lona, el corte deja hasta dos
-- piezas físicas: una franja de ancho distinto al original (un resto genuino,
-- subproducto de este corte concreto) y, cuando el corte no agota el largo de
-- la pieza, un remanente que conserva exactamente el mismo ancho que la pieza
-- original: eso no es un "resto generado para este pedido", es sencillamente
-- la misma pieza de origen que continúa existiendo, más corta. Hasta ahora
-- ambas piezas quedaban con source_sales_order_id/source_sales_order_line_id/
-- source_work_sheet_id/source_work_sheet_line_id apuntando al pedido y a la
-- hoja que disparó el corte, como si las dos hubieran sido "generadas para"
-- ese pedido. Solo la franja de ancho distinto (el resto genuino) debe llevar
-- esa trazabilidad; la pieza que solo se acorta longitudinalmente mantiene su
-- parentesco (parent_stock_item_id) pero vuelve a stock general, sin ligar a
-- ningún pedido/hoja concreto.
  ...
  for v_line in select * from public.production_work_sheet_line where work_sheet_id=p_work_sheet_id order by line_no loop
    ...
    for v_cw,v_ch in select (x->>0)::numeric,(x->>1)::numeric from jsonb_array_elements(v_remnants) x loop
      ...
      if greatest(v_cw,v_ch)>=v_min and v_cw>0 and v_ch>0 then
        ...
        values(v_item.warehouse_stock_id,v_item.product_id,v_item.characteristic_id,1,jsonb_build_array(v_cw,v_ch),v_remnant_units,'AVAILABLE',v_item.id,null,v_ord_id,v_ord_line_id,v_ws_id,v_ws_line_id,now()) returning id into v_new_item;
