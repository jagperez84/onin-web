-- ONIN: register_stock_movement() rompía con "null value in column dimension_values
-- violates not-null constraint" en cualquier consumo NO dimensional (p. ej. el nuevo
-- descuento de componentes por unidades, que llama con p_dimension_values = null).
--
-- v_dimension_array se inicializa a '[]'::jsonb y solo se rellena cuando
-- p_dimension_values es un objeto {codigo: valor} (entrada dimensional desde el
-- formulario). Cuando no lo es (el caso normal: null, o ya venía como array), la
-- expresión `case when jsonb_array_length(v_dimension_array)>0 then v_dimension_array
-- else p_dimension_values end` caía a p_dimension_values tal cual — que es NULL en el
-- caso no dimensional, y la columna es NOT NULL. Se corrige con coalesce(...,'[]'::jsonb)
-- para que solo sustituya cuando realmente es null, preservando el comportamiento para
-- quien ya pasa un array de dimensiones directamente.
...
begin
  ...
  insert into public.stock_movement(company_id,warehouse_id,product_id,movement_type_id,characteristic_id,quantity,movement_date,reference,notes,transfer_group_id,dimension_values,dimension_units)
  values(p_company_id,p_warehouse_id,p_product_id,v_type.id,p_characteristic_id,p_quantity,p_movement_date,p_reference,p_notes,p_transfer_group_id,case when jsonb_array_length(v_dimension_array)>0 then v_dimension_array else coalesce(p_dimension_values,'[]'::jsonb) end,v_dimension_units)
  ...
  if v_signed>0 and jsonb_array_length(v_dimension_array)>0 and coalesce(v_product.include_measurements_in_stock,false) then
    ...
    for v_item_index in 1..v_item_quantity loop
      insert into public.warehouse_stock_item(warehouse_stock_id,product_id,characteristic_id,quantity,dimension_values,dimension_units,status,source_stock_movement_id)
      values(v_stock.id,p_product_id,p_characteristic_id,1,v_dimension_array,v_dimension_units,'AVAILABLE',v_movement_id);
    end loop;
  end if;
