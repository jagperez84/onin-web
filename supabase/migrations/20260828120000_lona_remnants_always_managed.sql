-- ONIN: gestionar restos/mermas de lona para cualquier producto confeccionable,
-- no solo cuando product_family.recuttable está activo.
--
-- execute_lona_confection_work_sheet() solo se invoca sobre hojas
-- document_type = 'LONA_CONFECTION', es decir, artículos confeccionables por
-- definición. El resto que deja cada corte (v_remnants) siempre se calculaba y
-- se guardaba en production_work_sheet_line.remainder_pieces, pero la
-- materialización física de esos restos (nueva warehouse_stock_item AVAILABLE
-- si supera minimum_remainder, o registro de merma DIMENSIONAL_SCRAP en caso
-- contrario) estaba envuelta en `if v_recuttable then ... end if;`. Cuando la
-- familia del artículo no tenía recuttable=true (el caso habitual: LON-ACR-001
-- no lo tenía), el resto se calculaba pero nunca se creaba ni como existencia
-- ni como movimiento de stock, así que desaparecía sin dejar rastro en
-- Existencias ni en Movimientos de stock. Ese `if` es el error: recuttable
-- decide si un retal puede *reutilizarse para futuros cortes*, no si hay que
-- *registrar* lo que ha quedado del corte. Se retira la condición: el bloque
-- de restos/merma pasa a ejecutarse siempre para estas hojas.
...
declare
  ...
  v_w numeric; v_h numeric; v_cw numeric; v_ch numeric; v_remnants jsonb; v_min numeric:=0; v_rotated boolean;
