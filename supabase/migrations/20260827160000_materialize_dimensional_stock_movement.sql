...
begin
  ...
  if p_dimension_values is not null
     ...
     and (select count(*) from jsonb_each(p_dimension_values))>0 then
    ...
    if v_measurement_type_id is not null then
      select
        coalesce(jsonb_agg(case when p_dimension_values ? mtd.code then to_jsonb((p_dimension_values->>mtd.code)::numeric) else 'null'::jsonb end order by mtd.dimension_number),'[]'::jsonb),
        coalesce(jsonb_agg(coalesce(u.code,u.name) order by mtd.dimension_number),'[]'::jsonb),
        count(*)
      into v_dimension_array,v_dimension_units,v_dimension_count
      from public.measurement_type_dimension mtd left join public.unit u on u.id=mtd.unit_id
  ...
  v_signed:=p_quantity*p_type.direction;
  ...
  values(p_company_id,p_warehouse_id,p_product_id,v_type.id,p_characteristic_id,p_quantity,p_movement_date,p_reference,p_notes,p_transfer_group_id,case when jsonb_array_length(v_dimension_array)>0 then v_dimension_array else p_dimension_values end,v_dimension_units)
