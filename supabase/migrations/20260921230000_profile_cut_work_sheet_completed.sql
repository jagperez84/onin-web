-- El corte de perfil se ejecuta de forma atómica dentro de la misma transacción que crea la
-- hoja de trabajo (execute_manual_dimensional_cut se llama ANTES del insert en
-- production_work_sheet, ver 20260921170000_restore_profile_cut_remnant_traceability.sql):
-- el material ya está físicamente cortado y descontado del stock en el momento en que el
-- documento existe. Crearlo con status='ISSUED' ("pendiente de taller") es incorrecto — no hay
-- ningún paso posterior de taller para PROFILE_CUT, a diferencia de LONA_CONFECTION y
-- COMPONENT_CONSUMPTION, que sí se marcan COMPLETED al crearse. Como nada en el código transiciona
-- jamás una hoja de corte de perfil de ISSUED a COMPLETED, quedaban etiquetadas "Emitida" para
-- siempre aunque el trabajo ya estuviera hecho.
...
begin
  ...
  insert into public.production_work_sheet (
    ...
    p_company_id, v_code, 'PROFILE_CUT', now(), 'COMPLETED',
...
-- Backfill: las hojas de corte de perfil ya existentes en ISSUED representan cortes que también
-- se ejecutaron de forma atómica en su momento (esta función nunca ha tenido un paso de taller
-- separado) — reflejan trabajo ya hecho, no pendiente.
update public.production_work_sheet
set status = 'COMPLETED'
where document_type = 'PROFILE_CUT' and status = 'ISSUED';
