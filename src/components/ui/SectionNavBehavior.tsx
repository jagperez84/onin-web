...
function setupProductTargets() {
  const panels = Array.from(
    document.querySelectorAll<HTMLElement>(
      ".product-detail-grid > .panel, .product-profile-section-wrap > .panel",
    ),
  );
  ...
  const headingMap: Array<[string, string]> = [
    ...
    ["dimensiones heredadas", "producto-dimensiones"],
    ["tipo de medida", "producto-dimensiones"],
    ...
    ["proveedores y precios", "producto-precios"],
  ];
...
function updateActive(nav: HTMLElement) {
  const links = Array.from(
    nav.querySelectorAll<HTMLAnchorElement>("a[data-section-target]"),
  );
    ...
    const scan = () => {
      document
        .querySelectorAll<HTMLElement>(
          ".customer-section-nav, .product-profile-section-nav",
        )
        .forEach((nav) => {
          setupNav(nav);
          if (nav.classList.contains("product-profile-section-nav"))
