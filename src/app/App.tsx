...
const implementedRoutes = [
  '/ventas/clientes','/ventas/articulos','/gestion/mediciones','/configuracion/usuarios',
    ...
    <div className="app-shell">
      ...
      <div className="workspace">
        ...
        <main className="content"><Routes>
          ...
          <Route path="/configuracion/usuarios" element={<UserList />} /><Route path="/configuracion/usuarios/:id" element={<UserDetail />} />
...
function HomeView() {
  const { user } = useAuth(); const [assigned, setAssigned] = useState<AssignedMeasurementRow[]>([]);
  useEffect(() => { let active=true; if(!user?.id){setAssigned([]);return()=>{active=false;};} void listAssignedMeasurements(user.id).then(rows=>{if(active)setAssigned(rows);}).catch(()=>{if(active)setAssigned([]);}); return()=>{active=false;}; },[user?.id]);
  const cards=[['Clientes','Gestión de clientes','/ventas/clientes'],['Artículos','Catálogo de productos','/ventas/articulos'],['Presupuestos','Gestor de presupuestos','/ventas/presupuestos']];
