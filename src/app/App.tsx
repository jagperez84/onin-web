          ...
          {navSections.map(section => (
            <div key={section.label} className="nav-section">
              ...
              {section.items.map(item => (
                <NavLink key={item.to} to={item.to} className={({isActive}) => `nav-link ${isActive ? 'active' : ''}`} onClick={() => setMobileOpen(false)}>
        ...
        <main className="content">
          <Routes>
            ...
            {navSections.flatMap(s => s.items)
              .filter(item => !['/ventas/clientes','/ventas/articulos','/gestion/mediciones','/almacen/almacenes','/almacen/existencias','/almacen/movimientos','/almacen/transferencias','/almacen/reservas'].includes(item.to))
              .map(item => <Route key={item.to} path={item.to} element={<PagePlaceholder title={item.label}/>}/>)}
...
function HomeView() {
  ...
  return (
    <div className="home-page">
      ...
      <div className="metric-grid">{cards.map(([t,d,to]) => <NavLink key={t} to={to} className="metric-card"><span className="metric-title">{t}</span><span className="metric-desc">{d}</span><span className="metric-arrow">→</span></NavLink>)}</div>
      <CoreStatus/>
