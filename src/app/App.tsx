    ...
    <div className="app-shell">
      ...
      <div className="workspace">
        <header className="topbar">
          <button className="mobile-menu" onClick={() => setMobileOpen(v => !v)} aria-label="Abrir menú">
            <Menu size={20} />
          </button>
          <div className="crumb">
          ...
          <div className="global-search">
            <Search size={16} />
            <input placeholder="Buscar..." aria-label="Buscar en ONIN" />
          </div>
          <button className="avatar" title={user?.email ?? 'Usuario'}>
      ...
      {mobileOpen && (
        <button className="scrim" onClick={() => setMobileOpen(false)} aria-label="Cerrar menú" />
      )}
    </div>
  );
}
    ...
    <div className="home-page">
      <div className="page-head">
        <div>
          ...
          <p>Acceso rápido a las áreas principales de ONIN.</p>
        </div>
      </div>
      <div className="metric-grid">
        {cards.map(([t, d, to]) => (
          <NavLink key={t} to={to} className="metric-card">
            <span className="metric-title">{t}</span>
            <span className="metric-desc">{d}</span>
      ...
      <div className="panel">
        <div className="panel-head">
          <div>
            ...
            <p>Fundación técnica con autenticación preparada.</p>
          </div>
          <span className="status-pill">FASE 2</span>
        </div>
