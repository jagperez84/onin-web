...
const MATH_FUNCTIONS: TokenItem[] = [
  {
    ...
    type: "FUNCTION",
    detail: "CEIL(n) · Ej. CEIL(ANCHO / 1000)",
  },
  {
    code: "ROUND()",
    name: "Redondeo estándar al entero más próximo",
    type: "FUNCTION",
    detail: "ROUND(n) · Ej. ROUND(SALIDA / 500)",
  },
  {
    code: "FLOOR()",
    name: "Redondeo hacia abajo",
    type: "FUNCTION",
    detail: "FLOOR(n) · Ej. FLOOR(ANCHO / 1000)",
  },
  {
    code: "MAX()",
    name: "Valor máximo entre dos expresiones",
    type: "FUNCTION",
    detail: "MAX(a, b) · Ej. MAX(ANCHO, 2000)",
  },
  {
    code: "MIN()",
    name: "Valor mínimo entre dos expresiones",
    type: "FUNCTION",
    detail: "MIN(a, b) · Ej. MIN(SALIDA, 3000)",
  },
];
...
export function FormulaPredictiveInput({
    ...
    for (const inp of availableInputs) {
      if (inp.code?.trim()) {
        list.push({
          ...
          detail: inp.selection_type
            ? `Entrada ${inp.selection_type}`
            : "Entrada oficina",
        });
      }
    }
  ...
  return (
    <div
      className={`otd-formula-field-container ${className}`}
      ref={containerRef}
    >
      ...
      {label && (
        <div className="otd-formula-field-head">
          ...
          <button
            ...
            title={
              showAssistant
                ? "Ocultar herramienta de cálculo"
                : "Mostrar asistente de fórmulas"
            }
          >
            <Sparkles size={11} />
                ...
                return (
                  ...
                  >
                    <div className="sugg-left">
                      {item.type === "INPUT" && (
                        <Tag size={13} className="sugg-icon input" />
                      )}
                      {item.type === "VARIABLE" && (
                        <Zap size={13} className="sugg-icon var" />
                      )}
                      {item.type === "FUNCTION" && (
                        <Calculator size={13} className="sugg-icon func" />
                      )}
                    ...
                    <div className="sugg-right">
                      ...
                      {item.type === "VARIABLE" && (
                        <span className="sugg-badge var">Variable</span>
                      )}
                      {item.type === "FUNCTION" && (
                        <span className="sugg-badge func">Función</span>
                      )}
                      {item.detail && (
                        <span className="sugg-detail">{item.detail}</span>
                      )}
                    </div>
                  </button>
                );
      ...
      {showAssistant && (
        <div
          className={`otd-formula-assistant-drawer ${compact ? "compact" : ""}`}
        >
            ...
            <div className="assistant-group">
              ...
              <div className="assistant-chips">
                ...
                {availableInputs.length === 0 && (
                  <span className="assistant-empty-note">
                    No hay entradas definidas
                  </span>
                )}
              </div>
            </div>
