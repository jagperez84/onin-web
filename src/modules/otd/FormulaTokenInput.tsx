...
function renderExpression(value: string, variables: FormulaVariable[]) {
  ...
  return value
    ...
    .map((part, i) =>
      codes.includes(part)
        ? `<span class="formula-token" data-token="${escapeHtml(part)}" contenteditable="false">${escapeHtml(part)}</span>`
        : escapeHtml(part),
    )
    .join("");
}
...
function readExpression(root: HTMLElement) {
  ...
  root.childNodes.forEach((node) => {
    if (
      ...
      out += (node as HTMLElement).dataset.token;
    else out += node.textContent ?? "";
  });
  return out;
}

export function FormulaTokenInput({
  value,
  onChange,
  variables,
  placeholder,
  ...
  return (
    <div
      className={`formula-token-editor ${className} ${focused ? "is-focused" : ""}`}
    >
      <div
        ref={ref}
        className="formula-token-input"
        contentEditable
        suppressContentEditableWarning
        data-placeholder={placeholder || "Escribe una fórmula…"}
        onFocus={() => setFocused(true)}
        onBlur={() => setTimeout(() => setFocused(false), 120)}
        onInput={handleInput}
      />
      {focused && filtered.length > 0 && (
        <div className="formula-token-suggestions">
          {filtered.map((v) => (
            <button
              type="button"
              key={v.code}
            ...
            >
              <span className="formula-token-suggestion-chip">{v.code}</span>
              <span>
                <strong>{v.name}</strong>
                {v.expression && <small>{v.expression}</small>}
              </span>
            </button>
          ))}
        </div>
