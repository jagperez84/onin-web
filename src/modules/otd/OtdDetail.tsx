...
export function OtdDetail() {
  ...
  const [success, setSuccess] = useState((location.state as RouteState | null)?.success ?? '');
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const productIds = [
          ...new Set(rawComps.map(x => x.product_id).filter((x: any) => Number.isFinite(x))),
        ];
    ...
    <div className="otd-page">
      ...
      <div className="otd-head">
        <div>
          ...
          <h1>{otd.name}</h1>
          <p>
            {otd.code} · {templateLabelMap[otd.template_type ?? ''] || otd.template_type || 'Configurador genérico'}
          </p>
        </div>
      ...
      <div className="otd-editor-layout">
        <div className="otd-top-nav-wrapper">
          <nav className="otd-top-navigator" aria-label="Navegación de secciones">
        ...
        <div className="otd-editor-form-col">
          ...
          <section id="sec-identificacion" className="otd-card otd-section-anchor">
            <div className="otd-card-head">
              ...
              <span className={`otd-detail-badge ${otd.active ? 'active' : 'inactive'}`}>
                {otd.active ? (
                  <>
            ...
            <div className="otd-grid four">
              ...
              <div className="otd-detail-val-box">
                ...
                <div className="val-text">
                  {templateLabelMap[otd.template_type ?? ''] || otd.template_type || 'Genérico'}
                </div>
              </div>
              <div className="otd-detail-val-box">
          ...
          <section id="sec-entradas" className="otd-card otd-section-anchor">
            <div className="otd-card-head">
              <div>
                ...
                <p>
                  Parámetros que el usuario de ventas / presupuestos seleccionará al configurar el producto.
                </p>
              </div>
            </div>

            {selections.length === 0 ? (
              <div className="otd-empty">No hay entradas de oficina definidas en este OTD.</div>
            ) : (
              selections.map((s, si) => (
                <div className="otd-row-card" key={s.id || si}>
                  <div className="otd-row-actions">
                    <span className="row-tag">
                      ...
                      {s.required && <span className="nav-badge">Obligatorio</span>}
                    </span>
                  </div>
                  ...
                  {s.selection_type === 'OPTION' && (
                    <div className="otd-options-subcard">
                      <div className="otd-options-subhead">
                        <strong>Opciones de selección ({s.options.length})</strong>
                      </div>
                      {s.options.length === 0 ? (
          ...
          <section id="sec-escalado" className="otd-card otd-section-anchor">
            <div className="otd-card-head">
              <div>
                ...
                <p>
                  El OTD tiene su propio escalado que determina el precio base del producto compuesto según dimensiones.
                </p>
              </div>
            </div>
                ...
                <table className="otd-scale-table">
                  ...
                  <tbody>
                    {scales.map((sc, sci) => (
                      <tr key={sc.id || sci}>
                        <td>{sci + 1}</td>
                        <td>{Number(sc.dimension_1).toLocaleString('es-ES')} mm</td>
                        <td>
                          {sc.dimension_2 != null
                            ? `${Number(sc.dimension_2).toLocaleString('es-ES')} mm`
                            : '—'}
                        </td>
                        <td>
                          <strong>
          ...
          <section id="sec-formulacion" className="otd-card otd-section-anchor">
            <div className="otd-card-head">
              <div>
                ...
                <p>
                  Variables técnicas y fórmulas intermedias definidas para el cálculo del producto.
                </p>
              </div>
            ...
            {variables.length === 0 ? (
              <div className="otd-empty">No hay variables calculadas definidas en este OTD.</div>
            ) : (
          ...
          <section id="sec-componentes" className="otd-card otd-section-anchor">
            <div className="otd-card-head">
              <div>
                ...
                <p>
                  Artículos reales de ONIN vinculados al OTD que forman el despiece, cálculo de medidas de corte y costes.
                </p>
              </div>
            </div>

            {components.length === 0 ? (
              <div className="otd-empty">No hay componentes vinculados al OTD.</div>
            ) : (
              components.map((c, ci) => {
                const product = c.product;
                const dimensions = product?.measurement_type?.dimensions ?? [];
                const characteristics = product?.characteristics ?? [];
                const isDynamicChar = Boolean(c.characteristic_expression?.trim());
                ...
                return (
                  <div className="otd-row-card" key={c.id || ci}>
                        ...
                        <div className="val-text">
                          {c.component_type === 'IMPROVEMENT' ? (
                            c.price_increment_type === 'PERCENTAGE' ? (
                              ...
                              <strong>
                                +{Number(c.price_increment).toLocaleString('es-ES', {
                                  style: 'currency',
                                  currency: 'EUR',
                                })}
                              </strong>
                            ...
                            <span style={{ color: '#64748b' }}>Incluido en base (€ 0)</span>
                          )}
                        </div>
                      </div>
                    </div>
                    ...
                    {(characteristics.length > 0 || isDynamicChar || c.characteristic_id) && (
                      <div className="otd-detail-char-box">
                        <small>Característica / Color del Componente:</small>
                        {isDynamicChar ? (
                          <span>
                            Dinámica por variable/entrada: <code>{c.characteristic_expression}</code>
                          </span>
                        ) : selectedChar ? (
                          <span>
                            Fija: <strong>{selectedChar.code}</strong>
                            {selectedChar.description ? ` · ${selectedChar.description}` : ''}
                          </span>
                        ) : (
                          <span style={{ color: '#64748b' }}>Sin característica seleccionada</span>
                        )}
                      </div>
                    )}
                    ...
                    {dimensions.length > 0 && (
                      <div className="otd-detail-dims-wrap">
                        <div className="otd-detail-dims-title">
                          <Ruler size={14} />
                          <span>Dimensiones del artículo para corte y fabricación:</span>
                        </div>
                        <div className="otd-detail-dims-grid">
                          {dimensions.map(d => (
                            <div key={d.id} className="otd-detail-dim-item">
                              <label>
                                {d.name}
                                {d.unit?.name || d.unit?.code ? ` (${d.unit?.name || d.unit?.code})` : ''}
                              </label>
                              <code>{c.dimension_expressions?.[d.code] || '—'}</code>
                            </div>
                          ))}
                        </div>
                      </div>
