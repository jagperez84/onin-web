...
export function OtdRuntime() {
  ...
  const [quotations, setQuotations] = useState<Array<{ id: number; code: string; customer_name?: string }>>([]);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (active) setError(err?.message || 'Error al cargar el configurador OTD.');
      } finally {
    ...
    (async () => {
      try {
        const { data } = await supabase
          .from('quotation')
          .select('id, code, customer:customer_id(party:party_id(trade_name, legal_name))')
          .eq('status', 'DRAFT')
        ...
        if (data) {
          setQuotations(
            data.map((q: any) => ({
              ...
              customer_name: q.customer?.party?.trade_name || q.customer?.party?.legal_name || 'Cliente',
            }))
          );
        }
      } catch {
  ...
  const isCustomized = useMemo(() => {
    ...
    return JSON.stringify(customComponents) !== JSON.stringify(runtimeData.components);
  ...
  async function handleSelectProductForComp(product: Product) {
    try {
      ...
      const updatedComp: OtdComponentDef = {
        ...
        code: product.code,
        description: product.commercial_description || product.technical_description || product.code,
        product: details.product,
        characteristics: details.characteristics,
        dimensions: details.dimensions,
        scales: details.scales,
        characteristic_id: details.characteristics.length > 0 ? details.characteristics[0].id : null,
      };
  ...
  const handleAddToQuote = async () => {
    ...
    try {
      ...
      const { error: insertError } = await supabase.from('quotation_line').insert({
      ...
      setToast(`Línea añadida con éxito al presupuesto ${selectedQuotationId}.`);
  ...
  return (
    <div className="otd-runtime-page">
      ...
      <div className="otd-runtime-head">
        <div>
          <NavLink to={`/produccion/otd/${otd.id}`} className="otd-runtime-back">
            <ArrowLeft size={15} /> Volver al Editor Técnico
          </NavLink>
          <div className="eyebrow">CONFIGURADOR OTD · OFICINA & PRUEBAS</div>
          <h1>{otd.name}</h1>
          <p>
            {otd.code} · Introduce las medidas y características. El sistema resolverá automáticamente el
            escalado del artículo, su precio base y los incrementos configurados. Puedes cambiar o añadir
            componentes libremente para esta prueba.
          </p>
        </div>

        <div className="otd-runtime-actions-top">
          {isCustomized ? (
            <div className="badge-customized-pill" title="Componentes adaptados para esta prueba">
          ...
          <section className="otd-runtime-card">
            <div className="runtime-card-head">
              <div>
                ...
                <p>Información requerida para el dimensionamiento y selección de acabados.</p>
              </div>
              ...
              {selections.map(s => (
                <label key={s.id} className="runtime-input-field">
                  <span>
                    ...
                    {s.is_dimension && <span className="tag-dimension">Dimensión</span>}
                  </span>
                  ...
                  {s.selection_type === 'OPTION' ? (
                    ...
                    <div className="input-with-unit">
                      <input
                        ...
                        className="runtime-input"
                      />
                      <span className="unit-label">{s.is_dimension ? 'mm' : 'ud'}</span>
                    </div>
                  )}
                </label>
              ))}
            ...
            {calculation?.requiredMissing && calculation.requiredMissing.length > 0 && (
          ...
          <section className="otd-runtime-card">
            <div className="runtime-card-head">
              <div>
                ...
                <p>
                  Incluye el escalado base del OTD y los componentes configurados. Puedes cambiar o
                  añadir componentes para esta prueba.
                </p>
              </div>
                ...
                <div className="comp-info">
                  ...
                  <div className="comp-subtext">
                    {calculation?.otdScaleStepUsed ? (
                      ...
                      <span>Matriz de escalado OTD ({scales.length} escalones configurados)</span>
                    ) : (
                      <span>OTD sin matriz de escalado directo (suma por componentes)</span>
                    )}
                  </div>
                </div>
                    ...
                    <div className="comp-info">
                      <div className="comp-title">
                        ...
                        {c.component_type === 'IMPROVEMENT' ? (
                          ...
                          <span className="badge-base-tag">Básico (incluido en base)</span>
                        )}

                        {isInactive && (
                          <span className="text-danger" style={{ fontSize: 11, fontWeight: 600 }}>
                      ...
                      <div className="comp-subtext">
                        ...
                        {c.dimension_list.length > 0 && (
                          <span>
                            ...
                            · Medidas:{' '}
                            <b>
                              {c.dimension_list.map(d => `${d.name}: ${d.value} mm`).join(' × ')}
                            </b>
                          </span>
                        )}
                      </div>
                      ...
                      {c.scale_step_used && (
                        <div className="comp-scale-pill">
                          ...
                          {c.scale_step_used.dimension_2 != null
                            ...
                            : ''}{' '}
                          mm &rarr; {euro(c.base_price)}
                        </div>
                      )}

                      {!c.ok && (
                        <div className="comp-error-msg">
                          {c.formula_error || 'No se pudo resolver el precio del escalado.'}
                        </div>
                      )}
                    </div>
                    ...
                    <div className="comp-right-wrap">
                      <div className="comp-pricing">
                        <div className="comp-unit-price">
                          {c.component_type === 'IMPROVEMENT' && c.increment_amount > 0
                            ? `+${euro(c.unit_price)}`
                            : euro(c.unit_price)}
                          {c.quantity > 1 && <small> / ud</small>}
                        </div>
                        <div className="comp-total-price">
                          <strong>
                            {c.component_type === 'IMPROVEMENT' && c.total_price > 0
                              ? `+${euro(c.total_price)}`
                              : euro(c.total_price)}
                          </strong>
                        </div>
                      ...
                      <div className="comp-actions-inline">
                        ...
                        <button
                          ...
                          title={isInactive ? 'Activar componente' : 'Desactivar componente'}
                        >
          ...
          <section className="otd-runtime-card technical-accordion">
            ...
            >
              <div className="toggle-left">
                <Calculator size={18} />
                <span>
                  <strong>Ver detalles técnicos del OTD</strong> (Variables internas, fórmulas y
                  matriz de escalado)
                </span>
              </div>
            ...
            {showTechnicalDetails && (
              <div className="technical-accordion-content">
                <div className="tech-section-title">Variables Calculadas en Runtime</div>
                <div className="runtime-variable-grid">
                  {variables.length === 0 ? (
                    <div className="runtime-muted">No hay variables calculadas en este OTD.</div>
                  ) : (
                    variables.map(v => (
                      <div key={v.id} className="tech-variable-card">
                        ...
                        <div className="var-expr">Fórmula: <code>{v.expression || '—'}</code></div>
                        <div className="var-val">
                          Valor:{' '}
                          <strong>
                            {Number.isFinite(calculation?.resolvedVariables[v.code])
                              ? calculation?.resolvedVariables[v.code].toLocaleString('es-ES', {
                ...
                <div className="tech-table">
                  ...
                  {calculation?.components.map((c, i) => (
                    <div key={i} className="tech-table-row">
                      ...
                      <span>
                        <span className={`tag-comp-type ${c.component_type.toLowerCase()}`}>
                          {c.component_type === 'BASIC' ? 'Básico' : 'Mejora'}
                        </span>
                      </span>
                      <span>
                        {c.dimension_list.length > 0
                          ? c.dimension_list.map(d => `${d.code}=${d.value}`).join(', ')
                          : 'Sin dimensiones'}
                      </span>
                      <span>
                        {c.scale_step_used
                          ? `${c.scale_step_used.dimension_1} × ${c.scale_step_used.dimension_2 ?? '—'} mm (${euro(c.base_price)})`
                          : euro(c.base_price)}
                      </span>
                      <span>
                        {c.component_type === 'IMPROVEMENT' && c.price_increment > 0
                          ? c.price_increment_type === 'PERCENTAGE'
            ...
            <div className="otd-modal-body">
                ...
                {productResults.length > 0 && (
                  <div className="search-results-list">
                    {productResults.map(p => (
                      ...
                      >
                        <div>
                          <strong>{p.commercial_description || p.technical_description || p.code}</strong>
                        </div>
              ...
              >
                ...
                <div style={{ fontSize: 12, color: '#64748b', marginTop: 2 }}>
                  Código: <code>{editingCompModal.comp.product?.code || editingCompModal.comp.code || '—'}</code>
                </div>
              </div>
                  ...
                  <select
                    ...
                    onChange={e =>
                      setEditingCompModal({
                        ...
                        comp: {
                          ...editingCompModal.comp,
                          component_type: e.target.value as 'BASIC' | 'IMPROVEMENT',
                        },
                      })
                    }
                    className="runtime-select"
                  >
                    <option value="BASIC">Básico (incluido en tarifa base)</option>
                    <option value="IMPROVEMENT">Mejora (con incremento de precio)</option>
                  </select>
                </label>
              </div>
                    ...
                    <select
                      ...
                      onChange={e =>
                        setEditingCompModal({
                          ...
                          comp: {
                            ...editingCompModal.comp,
                            price_increment_type: e.target.value as 'FIXED' | 'PERCENTAGE',
                          },
                        })
                      }
                      className="runtime-select"
      ...
      {showAddToQuoteModal && (
        <div className="otd-modal-overlay" role="dialog" aria-modal="true">
          <div className="otd-modal-box">
            ...
            <div className="otd-modal-body">
              <p>
                Se generará un <strong>snapshot inmutable</strong> con las medidas, componentes
                personalizados, escalados e incrementos calculados en este momento.
              </p>
              ...
              <div className="quote-summary-mini">
                <div>
                  ...
                  <div className="text-muted" style={{ fontSize: '13px' }}>
                    {snapshot?.inputs_display.map(i => `${i.name}: ${i.display_value}`).join(' · ')}
                  </div>
                </div>
              ...
              <div className="quote-select-field">
                <label>
                  ...
                  <select
                    ...
                    onChange={e =>
                      setSelectedQuotationId(e.target.value ? Number(e.target.value) : null)
                    }
                  >
                    <option value="">Crear un NUEVO Presupuesto con esta configuración</option>
                    {quotations.map(q => (
                ...
                <div>
                  ...
                  <ul className="snapshot-list">
                    {snapshot.inputs_display.map(i => (
                      <li key={i.code}>
                        <span>{i.name}:</span> <strong>{i.display_value}</strong>
                      </li>
                    ))}
                  </ul>
                </div>
                ...
                <div>
                  ...
                  <ul className="snapshot-list">
                    {snapshot.variables_display.map(v => (
                      <li key={v.code}>
                        <span>{v.name} ({v.code}):</span> <strong>{v.value}</strong>
                      </li>
                    ))}
                  </ul>
                </div>
