...
type Component = OtdComponentFormula & { id:number; product_id:number|null; description:string|null; component_type:string; price_increment:number; active:boolean; sort_order:number; dimension_expressions:Record<string,string>; product?:Product|null };
...
export function OtdRuntime() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        setComponents(componentRows.map(x=>({...x,product:productMap[x.product_id]??null,dimension_expressions:x.dimension_expressions&&typeof x.dimension_expressions==='object'?x.dimension_expressions:{}})) as Component[]);
      } catch (e:any) {
  ...
  const evaluation = useMemo(() => {
    const rows = components.map(c => {
      ...
      } catch (e:any) { return {...c,ok:false,quantity:0,dimensions:{},unit:undefined,unitPrice:0,total:0,formulaError:e?.message??'Fórmula no válida'}; }
    });
