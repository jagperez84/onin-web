...
import { Calculator, CheckCircle2, Copy, Edit3, Plus, RotateCcw, Save, Search, Sparkles, X } from 'lucide-react';
...
const emptyDimension = (n: number, unitId: number | null = null): MeasurementDimension => ({
  dimension_number: n,
  code: `DIM${n}`,
  name: n === 1 ? 'Ancho' : n === 2 ? 'Alto' : n === 3 ? 'Profundidad' : `Dimensión ${n}`,
  unit_id: unitId,
  decimals: 2,
});
...
export function MeasurementTypesV1() {
  ...
  function startNew() {
    const defaultUnit = units.find(u => u.code.toLowerCase() === 'mm' || u.code.toLowerCase() === 'm')?.id ?? units[0]?.id ?? null;
    const blank = emptyType();
    blank.dimensions = [emptyDimension(1, defaultUnit), emptyDimension(2, defaultUnit)];
    openEditor(blank);
  }
  ...
  function duplicateRow(row: MeasurementType) {
    const duplicated: MeasurementType = {
      ...
      name: `${row.name} (Copia)`,
      dimensions: row.dimensions.map((d, i) => ({ ...d, id: undefined, measurement_type_id: undefined, dimension_number: i + 1 })),
    };
    openEditor(duplicated);
  }

  function closeEditor() {
    if (dirty && !window.confirm('Hay cambios sin guardar en el tipo de medida. ¿Quieres descartarlos?')) return;
    setEditing(false);
    setError('');
  }
  ...
  function changeCount(count: number) {
    ...
    setForm(f => ({
      ...
      dimensions: Array.from({ length: safeCount }, (_, i) => f.dimensions[i] ?? emptyDimension(i + 1, defaultUnit)),
    }));
  }

  function applyPreset(preset: '1d' | '2d' | '3d') {
    const mmUnit = units.find(u => u.code.toLowerCase() === 'mm')?.id ?? units[0]?.id ?? null;
    const mUnit = units.find(u => u.code.toLowerCase() === 'm')?.id ?? units[0]?.id ?? null;
    const m2Unit = units.find(u => u.code.toLowerCase() === 'm2')?.id ?? units[0]?.id ?? null;
    const m3Unit = units.find(u => u.code.toLowerCase() === 'm3')?.id ?? units[0]?.id ?? null;
    ...
    if (preset === '1d') {
      setForm(f => ({
        ...
        formula: 'DIM1 / 1000',
        dimensions: [
          { dimension_number: 1, code: 'DIM1', name: 'Longitud', unit_id: mmUnit, decimals: 2 },
        ],
      }));
    } else if (preset === '2d') {
      setForm(f => ({
        ...
        dimensions: [
          { dimension_number: 1, code: 'DIM1', name: 'Ancho', unit_id: mmUnit, decimals: 2 },
          { dimension_number: 2, code: 'DIM2', name: 'Alto', unit_id: mmUnit, decimals: 2 },
        ],
      }));
    } else if (preset === '3d') {
      setForm(f => ({
        ...
        dimensions: [
          { dimension_number: 1, code: 'DIM1', name: 'Ancho', unit_id: mmUnit, decimals: 2 },
          { dimension_number: 2, code: 'DIM2', name: 'Alto', unit_id: mmUnit, decimals: 2 },
          { dimension_number: 3, code: 'DIM3', name: 'Profundidad', unit_id: mmUnit, decimals: 2 },
        ],
      }));
    }
  }
  ...
  function insertIntoFormula(token: string) {
    setForm(f => {
      ...
      const spacer = currentFormula.length > 0 && !currentFormula.endsWith(' ') && !token.startsWith(' ') ? ' ' : '';
      return {
        ...f,
  ...
  async function save() {
    ...
    try {
      ...
      setError(
        message.includes('measurement_type_company_code_uk') || message.includes('duplicate key')
  ...
  return (
    <div className="module-page catalog-page measurement-types-page" id="measurement-types-container">
      <div className="page-head measurement-header-bar" id="measurement-types-head">
        <div>
          <div className="eyebrow">VENTAS / CONFIGURACIÓN</div>
          <h1>Tipos de medida</h1>
          <p>Estructuras dimensionales, variables y fórmulas de cálculo reutilizables por artículos y presupuestos.</p>
        </div>
        <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
          <button className="primary-button" id="btn-new-measurement-type" onClick={startNew}>
      ...
      {successMessage && (
        <div className="auth-warning" style={{ background: 'var(--success-soft)', borderColor: 'var(--success)', color: '#276749', marginBottom: '16px', display: 'flex', alignItems: 'center', gap: '8px' }}>
          <CheckCircle2 size={16} /> {successMessage}
        </div>
      )}

      <div className={`catalog-layout ${editing ? 'has-editor' : ''}`} id="measurement-types-layout">
        {/* Main table container spanning the screen */}
        <section className="panel" id="measurement-types-table-panel" style={{ width: '100%' }}>
          <div className="panel-head">
            <div>
              <h2>Catálogo de tipos de medida</h2>
              <p>Gestiona dimensiones, unidades de entrada y fórmulas de conversión a unidades de resultado.</p>
            </div>
            <span className="measurement-stats-badge">
              {rows.length} {rows.length === 1 ? 'tipo configurado' : 'tipos configurados'}
            </span>
          </div>
          ...
          <div className="measurement-toolbar">
            <div className="measurement-search-wrap">
              <div className="search-field" style={{ width: '100%' }}>
                ...
                />
                {search && (
                  <button type="button" className="search-clear" onClick={() => setSearch('')} title="Limpiar búsqueda" aria-label="Limpiar búsqueda">
                    <X size={14} />
                  </button>
                )}
              </div>
            </div>
            <button className="secondary-button compact" onClick={load} title="Recargar lista">
              <RotateCcw size={14} /> Actualizar
            </button>
          </div>

          <div className="catalog-table-wrap">
            <table className="catalog-table measurement-table" id="measurement-types-table">
              <thead>
                <tr>
                    ...
                    <tr key={r.id} className={isCurrent ? 'is-editing' : ''}>
                      ...
                      <td>
                        <div className="measurement-title-cell">
                          ...
                          {r.calculation_type && <span className="measurement-subtitle">{r.calculation_type}</span>}
                        </div>
                      </td>
                      <td>
                        {r.dimensions.length ? (
                          <div className="dimension-pill-list">
                            {r.dimensions.map(d => (
                              <span className="dimension-badge" key={d.dimension_number} title={`${d.code}: ${d.name} (${unitCode(d.unit_id)})`}>
                                <span className="dimension-var">{d.code}</span>
                                <span>{d.name}</span>
                                <span className="dimension-unit-tag">{unitCode(d.unit_id) || 's/u'}</span>
                              </span>
                            ))}
                          </div>
                        ) : (
                          <span className="dim-zero-badge">0 dimensiones (unidad simple)</span>
                        )}
                      </td>
                      <td>
                        {r.calculation_type && <div className="calc-type-badge">{r.calculation_type}</div>}
                        {r.formula ? (
                          <div className="formula-code-badge" title={r.formula}>
                            fx: {r.formula}
                          </div>
                        ) : (
                          !r.calculation_type && <span style={{ color: 'var(--muted)' }}>—</span>
                        )}
                      </td>
                      <td>
                        <div className="result-badge">
                          <span className="result-unit-name">
                            {r.result_unit_id ? `${unitCode(r.result_unit_id)} · ${unitLabel(r.result_unit_id)}` : 'Sin unidad'}
                          </span>
                          <span className="result-unit-decimals">{r.result_decimals} decimales</span>
                        </div>
                      </td>
                      <td>
                        <span className={`status ${r.active ? 'active' : 'inactive'}`}>
                  ...
                  <tr>
                    <td colSpan={7}>
                      <div className="empty-state">
                        {search ? 'No hay tipos de medida que coincidan con la búsqueda.' : 'No hay tipos de medida definidos.'}
                      </div>
                    </td>
                  </tr>
                )}
        ...
        {editing && (
          <aside className="panel catalog-editor" id="measurement-type-editor-panel">
            <div className="panel-head">
              <div>
                <h2>{form.id ? `Editar: ${form.code || form.name}` : 'Nuevo tipo de medida'}</h2>
                <p>Estructura de entrada dimensional y reglas de cálculo del resultado.</p>
              </div>
              <button className="icon-action" onClick={closeEditor} title="Cerrar editor" aria-label="Cerrar editor">
                <X size={17} />
              </button>
            </div>
            ...
            <div className="form-grid">
              ...
              <label>
                Nº de dimensiones
                <select value={form.dimension_count} onChange={e => changeCount(Number(e.target.value))}>
                  {Array.from({ length: MAX_DIMENSIONS + 1 }, (_, n) => (
                    <option key={n} value={n}>
                      {n === 0 ? '0 dimensiones (fijo)' : `${n} ${n === 1 ? 'dimensión' : 'dimensiones'}`}
                    </option>
                  ))}
                </select>
              </label>
              ...
              {form.dimension_count > 0 && (
                <div className="wide editor-card-section">
                  <div className="editor-section-header">
                    <div className="editor-section-title">Dimensiones de entrada ({form.dimension_count})</div>
                    <div className="dimension-presets">
                      <button type="button" className="preset-chip-btn" onClick={() => applyPreset('1d')} title="Plantilla 1D: Longitud (mm)">
                        <Sparkles size={11} /> Preset 1D
                      </button>
                      <button type="button" className="preset-chip-btn" onClick={() => applyPreset('2d')} title="Plantilla 2D: Ancho × Alto (mm -> m²)">
                        <Sparkles size={11} /> Preset 2D (m²)
                      </button>
                      <button type="button" className="preset-chip-btn" onClick={() => applyPreset('3d')} title="Plantilla 3D: Ancho × Alto × Fondo (mm -> m³)">
                        <Sparkles size={11} /> Preset 3D (m³)
                      </button>
                    </div>
                  </div>
                  <p className="form-help">
                    Las variables <code>DIM1</code>, <code>DIM2</code>... se introducen en las líneas de presupuesto y alimentan la fórmula de cálculo.
                  </p>

                  <div style={{ display: 'grid', gap: '8px' }}>
                    {form.dimensions.map((d, i) => (
                      <div className="measurement-dimension-row" key={d.dimension_number}>
                        <div className="measurement-dimension-number">{i + 1}</div>
                        <label>
                          Variable
                          <input
                            value={d.code}
                            onChange={e => updateDimension(i, { code: e.target.value.toUpperCase() })}
                            maxLength={30}
                            placeholder={`DIM${i + 1}`}
                          />
                        </label>
                        ...
                        <label>
                          ...
                          <input
                            ...
                            step={1}
                            value={d.decimals}
                            onChange={e => updateDimension(i, { decimals: Number(e.target.value) })}
                          />
                        </label>
                      </div>
                    ))}
                ...
                <div className="form-grid">
                  ...
                  <label>
                    ...
                    <input
                      ...
                      onChange={e => setForm({ ...form, calculation_type: e.target.value.toUpperCase() })}
                  ...
                  <label className="wide">
                    ...
                    <input
                      ...
                      onChange={e => setForm({ ...form, result_decimals: Number(e.target.value) })}
                    />
                  </label>
                  ...
                  <div className="wide">
                    ...
                    <div className="formula-builder-bar">
                      ...
                      {form.dimensions.slice(0, form.dimension_count).map(d => (
                        <button
                          type="button"
                          key={d.code}
                      ...
                      <button type="button" className="formula-insert-btn operator" onClick={() => insertIntoFormula('+')}>+</button>
                      <button type="button" className="formula-insert-btn operator" onClick={() => insertIntoFormula('-')}>-</button>
                      <button type="button" className="formula-insert-btn operator" onClick={() => insertIntoFormula('*')}>*</button>
                      <button type="button" className="formula-insert-btn operator" onClick={() => insertIntoFormula('/')}>/</button>
                      <button type="button" className="formula-insert-btn operator" onClick={() => insertIntoFormula('(')}>(</button>
                      <button type="button" className="formula-insert-btn operator" onClick={() => insertIntoFormula(')')}>)</button>
                      <button type="button" className="formula-insert-btn operator" onClick={() => insertIntoFormula('1000')}>1000</button>
                      <button type="button" className="formula-insert-btn operator" onClick={() => insertIntoFormula('1000000')}>1000000</button>
                    </div>

                    <p className="form-help">
                      Fórmula que calcula la cantidad de resultado a partir de las dimensiones introducidas (p. ej. convertir mm a m² dividiendo por 1.000.000).
                    </p>
                  </div>
                </div>
              </div>
            ...
            <div className="editor-actions">
              ...
              <button className="primary-button" disabled={saving || !dirty} onClick={save} id="btn-save-measurement-type">
                <Save size={15} /> {saving ? 'Guardando...' : 'Guardar tipo de medida'}
              </button>
            </div>
          </aside>
        )}
