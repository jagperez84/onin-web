...
function isValidSpanishPhone(value:string){const p=normalizePhone(value);if(!p)return true;const local=p.startsWith('+34')?p.slice(3):p;return /^[6789]\d{8}$/.test(local);}
...
export function MeasurementCreate(){
  ...
  return <div className="module-page measurement-detail-page"><div className="page-head"><div><div className="eyebrow">GESTIÓN / MEDICIONES / NUEVA</div><h1>Nueva medición</h1><p>Crea el expediente con número automático y conserva el contexto de la visita.</p></div></div>
    ...
    <form onSubmit={save} className="measurement-main">
      ...
      <section className="panel"><div className="panel-head"><div><h2>Datos de Contacto</h2><p>Puede ser un cliente existente o un cliente potencial. Si no seleccionas un cliente, introduce aquí sus datos.</p></div></div><div className="form-grid contact-data-grid"><label>Nombre *<input value={contact.name} readOnly={hasCustomer} onChange={e=>updateContact('name',e.target.value)}/></label><label>Teléfono<input value={contact.phone} readOnly={hasCustomer} inputMode="tel" autoComplete="tel" onChange={e=>updateContact('phone',e.target.value)} placeholder="600 000 000"/></label><label>Móvil<input value={contact.mobile} readOnly={hasCustomer} inputMode="tel" autoComplete="tel-national" onChange={e=>updateContact('mobile',e.target.value)} placeholder="600 000 000"/></label><label className="wide">Email<input type="email" value={contact.email} readOnly={hasCustomer} onChange={e=>updateContact('email',e.target.value)}/></label></div></section>
      <section className="panel"><div className="panel-head"><div><h2>Ubicación de la medición</h2><p>La dirección se puede localizar mediante la API de direcciones, igual que en Clientes.</p></div><MapPin size={19}/></div><div className="form-grid measurement-address-grid"><AddressLookup value={address} onChange={setAddress}/><label className="measurement-address-wide">Dirección<input value={address.street} onChange={e=>setAddress(v=>({...v,street:e.target.value}))}/></label><label>CP<input value={address.postal_code} onChange={e=>setAddress(v=>({...v,postal_code:e.target.value}))}/></label><label>Localidad<input value={address.city} onChange={e=>setAddress(v=>({...v,city:e.target.value}))}/></label><label>Provincia<input value={address.region} onChange={e=>setAddress(v=>({...v,region:e.target.value}))}/></label></div></section>
