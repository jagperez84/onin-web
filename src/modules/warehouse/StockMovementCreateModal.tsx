...
export function StockMovementCreateModal({ onClose }: Props) {
  ...
  useEffect(() => {
    ...
    setError("");
    Promise.all([listStockCharacteristics(product.id), listProductDimensions(product.id)])
      .then(([characteristicRows, dimensionRows]) => {
  ...
  async function submit(event: FormEvent) {
    ...
    if (characteristicRequired && !characteristicId) {
      setError("Selecciona una característica para este artículo.");
      return;
    }
    ...
    try {
      const normalizedDimensions = Object.fromEntries(
        dimensions.map((dimension) => [
          dimension.code,
          dimensionValues[dimension.code] === undefined || dimensionValues[dimension.code] === "" ? null : Number(dimensionValues[dimension.code]),
        ])
      );
      if (dimensions.some((dimension) => normalizedDimensions[dimension.code] === null)) {
        throw new Error("Completa todas las dimensiones del artículo.");
      }
      ...
      await registerStockMovement({
        ...
        dimensionValues: normalizedDimensions,
        reference,
        notes,
        movementDate: date,
      });
      ...
      onClose();
      window.location.assign("/almacen/movimientos?stockSuccess=Movimiento%20registrado%20correctamente.");
    } catch (value) {
      ...
      <div className="stock-movement-modal" role="dialog" aria-modal="true" aria-labelledby="stock-movement-title">
        <div className="stock-modal-head">
          <div>
            ...
            <h2 id="stock-movement-title">Nuevo movimiento</h2>
            <p>Registra una entrada, salida o ajuste de stock.</p>
          </div>
        ...
        <form className="stock-form-grid" onSubmit={submit}>
          ...
          <label>
            ...
            <select value={type} onChange={(event) => setType(event.target.value)} required>
              <option value="">Seleccionar…</option>
              {types.map((movementType) => <option key={movementType.code} value={movementType.code}>{movementType.name}</option>)}
            </select>
          </label>
          ...
          <label>
            ...
            <select
              ...
              onChange={(event) => setCharacteristicId(event.target.value)}
              disabled={!product || characteristics.length === 0}
              required={Boolean(product?.include_stock_by_color && characteristics.length > 0)}
            >
              <option value="">{!product ? "Selecciona un artículo" : characteristics.length === 0 ? "Sin características asignadas" : "Sin característica"}</option>
              {characteristics.map((characteristic) => <option key={characteristic.id} value={characteristic.id}>{characteristic.code}{characteristic.description ? ` · ${characteristic.description}` : ""}</option>)}
            </select>
            {product && characteristics.length === 0 && <small className="field-hint">Este artículo no tiene características asignadas; no es necesario seleccionar ninguna.</small>}
          </label>

          {dimensions.length > 0 && (
            <div className="stock-dimension-fields field-wide">
              <div className="stock-dimension-title"><Ruler size={15} /> Dimensiones del artículo</div>
              <div className="stock-dimension-grid">
                {dimensions.map((dimension) => (
                  <label key={dimension.id}>
                    <span>{dimension.name} <small>({dimension.code}) *</small></span>
                    <div className="stock-dimension-input">
                      <input
                        type="number"
                        min="0"
                        step={dimension.decimals ? `0.${"0".repeat(Math.max(0, dimension.decimals - 1))}1` : "1"}
                        value={dimensionValues[dimension.code] ?? ""}
                        onChange={(event) => setDimensionValues((values) => ({ ...values, [dimension.code]: event.target.value }))}
                        required
                      />
                      <em>u.{dimension.unit_id}</em>
                    </div>
                  </label>
                ))}
              </div>
          ...
          <label>
            <span>Cantidad *</span>
            <input inputMode="decimal" type="number" min="0.0001" step="any" value={quantity} onChange={(event) => setQuantity(event.target.value)} required />
          </label>
          <label>
          ...
          <label>
            ...
            <input value={reference} onChange={(event) => setReference(event.target.value)} maxLength={255} />
          </label>
          ...
          <div className="stock-form-actions field-wide">
            <button type="button" className="stock-button" onClick={onClose}>Cancelar</button>
            <button type="submit" className="stock-button primary" disabled={saving}>{saving ? "Guardando…" : <><Save size={15} /> Registrar movimiento</>}</button>
          </div>
        </form>
      </div>
    </div>
