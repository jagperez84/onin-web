...
export function StockProductLookup({
  ...
  async function search() {
    ...
    setError("");
    try {
      setResults(await searchStockProducts(companyId, term));
      setOpen(true);
    } catch (e) {
      setError(
        e instanceof Error ? e.message : "No se pudieron buscar artículos.",
      );
    } finally {
      setLoading(false);
    }
    ...
    <div className="stock-lookup">
      <label>
        ...
        <div className="lookup-input">
          ...
          />
          <button type="button" title="Buscar" onClick={() => void search()}>
            <Search size={15} />
          </button>
          {value && (
            <button
      ...
      {open && !value && (
        <div className="lookup-results-stock">
          {loading && <small>Buscando…</small>}
          {!loading && results.length === 0 && (
            <small>No se encontraron artículos.</small>
          )}
          {results.map((p) => (
            <button
              type="button"
            ...
            >
              ...
              {!p.stock_enabled && <small>Stock no activado</small>}
            </button>
          ))}
        </div>
      )}
