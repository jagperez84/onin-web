...
function findTarget() {
  return document.querySelector<HTMLElement>(
    "#producto-datos-generales .form-grid",
  );
}
  ...
  useEffect(() => {
    ...
    async function load() {
      const { data: product } = await db
        .from("product")
        .select("family_id, measurement_type_id")
        .eq("id", Number(id))
        .maybeSingle();

      let effectiveTypeId: number | null = product?.measurement_type_id ?? null;

      if (!effectiveTypeId && product?.family_id) {
        const { data: family } = await db
          .from("product_family")
          .select("measurement_type_id")
          .eq("id", product.family_id)
          .maybeSingle();
        effectiveTypeId = family?.measurement_type_id ?? null;
      }

      if (cancelled || !effectiveTypeId) {
      ...
      if (!cancelled) {
        setValue(
          type ? { code: String(type.code), name: String(type.name) } : null,
        );
      }
    }
