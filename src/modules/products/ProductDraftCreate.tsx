...
export function ProductDraftCreate() {
  ...
  useEffect(() => {
    ...
    async function createDraft() {
      try {
        ...
        if (!cancelled)
          navigate(`/ventas/articulos/${id}?draft=1`, { replace: true });
      } catch (e) {
        if (!cancelled)
          setError(
            e instanceof Error
              ? e.message
              : "No se pudo iniciar el alta del artículo.",
          );
      }
    }
    createDraft();
  ...
  return (
    <div className="loading-block">{error || "Preparando nuevo artículo…"}</div>
  );
}
