...
export function ProductProfile(){
  ...
  useEffect(()=>{
    ...
    if(!shell) return;
    const captureError=()=>{
      const nodes=Array.from(shell.querySelectorAll<HTMLElement>('.inline-error')).filter(node=>!node.closest('.onin-message-log'));
      const node=nodes[0];
      const text=node?.textContent?.trim()||'';
      if(text&&text!==lastObservedError.current){lastObservedError.current=text;reportError(text);}
    ...
    <div className="product-profile-content">
      ...
      {!isNew&&<>
        <div className="product-profile-section-wrap">
          <ProductCharacteristicsPanel productId={Number(id)} readOnly={!editing} scaled={scaled} onSaved={()=>setRefreshKey(v=>v+1)} onError={reportError}/>
        </div>
        <div id="producto-precios" className="product-profile-section-wrap product-profile-anchor">
          <ProductCommercialPanel productId={Number(id)} editable={editing} scaled={scaled} refreshKey={refreshKey} onError={reportError}/>
        </div>
      </>}
    </div>
