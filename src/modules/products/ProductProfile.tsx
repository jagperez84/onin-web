...
export function ProductProfile() {
  ...
  useEffect(() => {
    ...
    const captureError = () => {
      const nodes = Array.from(
        shell.querySelectorAll<HTMLElement>(".inline-error"),
      ).filter((node) => !node.closest(".onin-message-log"));
      const node = nodes[0];
      const text = node?.textContent?.trim() || "";
      if (text && text !== lastObservedError.current) {
        lastObservedError.current = text;
        reportError(text);
      }
      if (!text) lastObservedError.current = "";
    };
    const observer = new MutationObserver(captureError);
  ...
  return (
    <div ref={shellRef} className="product-profile-shell">
      {!isNew && (
        <nav
          className="product-profile-section-nav"
          aria-label="Navegación rápida del artículo"
        >
          <a
            href="#producto-datos-generales"
            data-section-label="Datos generales"
            data-section-target="producto-datos-generales"
          >
            Datos generales
          </a>
          <a
            href="#producto-comercial"
            data-section-label="Comercial"
            data-section-target="producto-comercial"
          >
            Comercial
          </a>
          <a
            href="#producto-dimensiones"
            data-section-label="Dimensiones"
            data-section-target="producto-dimensiones"
          >
            Dimensiones
          </a>
          <a
            href="#producto-stock"
            data-section-label="Stock"
            data-section-target="producto-stock"
          >
            Stock
          </a>
          <a
          ...
          <a
            href="#producto-precios"
            data-section-label="Proveedores y precios"
            data-section-target="producto-precios"
          >
