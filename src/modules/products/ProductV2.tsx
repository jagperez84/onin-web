...
function ProductStatusBadge({
  ...
  product: Pick<Product, "active" | "deleted_at">;
}) {
  const deleted = !!product.deleted_at;
  return (
    <span
      className={`status ${deleted || !product.active ? "inactive" : "active"}`}
    >
      {deleted
...
export function ProductV2({
  ...
  async function loadList() {
    ...
    try {
      const [r, ref] = await Promise.all([
        listProducts(companyId, search, status),
        getProductReferences(companyId),
      ]);
      setRows(r);
      setRefs(ref);
    } catch (e) {
  ...
  async function startNew() {
    if (!companyId || creatingDraft) return;
    setCreatingDraft(true);
    setError("");
    try {
      const draftId = await createProductDraft(companyId);
      navigate(`/ventas/articulos/${draftId}?draft=1`);
    } catch (e) {
      setError(
        e instanceof Error
          ? e.message
          : "No se pudo preparar el nuevo artículo.",
      );
    } finally {
  ...
  if (id)
    return (
      <ProductEditor
        companyId={companyId}
        productId={isNew ? null : Number(id)}
        draftMode={isDraft}
        error={error}
        setError={setError}
        refs={refs}
    ...
    <div className="module-page product-page">
      <div className="page-head">
        <div>
          ...
          <p>Consulta y gestión del maestro de artículos.</p>
        </div>
        <div className="product-head-actions">
          ...
          >
            <Plus size={16} />{" "}
            {creatingDraft ? "Preparando…" : "Nuevo artículo"}
          </button>
        </div>
      </div>
      <div className="toolbar">
        ...
        <button className="secondary-button" onClick={loadList}>
          <RotateCcw size={15} /> Actualizar
        </button>
        <span className="result-count">{rows.length} artículos</span>
      </div>
              ...
              <tr>
                <td colSpan={8}>
                  <div className="empty-state">
                    ...
                    <span>Prueba otra búsqueda o crea un nuevo artículo.</span>
                  </div>
                </td>
              </tr>
            ) : (
                ...
                <tr key={r.id} className="clickable-row">
                  <td>
                    <button
                      ...
                      onClick={() => navigate(`/ventas/articulos/${r.id}`)}
                    >
                      {r.code}
                    </button>
                  </td>
                  <td>
                    <button
                      ...
                      onClick={() => navigate(`/ventas/articulos/${r.id}`)}
                    >
                      {r.commercial_description ||
                        r.technical_description ||
                        "—"}
                    </button>
                  </td>
                  <td>{r.family?.name || "—"}</td>
                  ...
                  <td>
                    {r.sales_price == null
                      ? "—"
                      : Number(r.sales_price).toFixed(2) + " €"}
                  </td>
                  <td>
                    {r.stock_enabled ? (
                      <>
                        <span className="stock-dot">●</span> Gestionado
                      </>
                    ) : (
                      "No gestionado"
                    )}
...
function ProductEditor({
  ...
  const [editing, setEditing] = useState(productId === null || draftMode);
  const [loading, setLoading] = useState(true);
  ...
  useEffect(() => {
    onScaledChange?.(!!form.scaled);
  }, [form.scaled, onScaledChange]);
  useEffect(() => {
    if (companyId) load();
  }, [companyId, productId]);
  async function load() {
    ...
    setError("");
    try {
      const referenceData = await getProductReferences(companyId);
      ...
      setError(
        e instanceof Error ? e.message : "No se pudo cargar el artículo.",
      );
    } finally {
      setLoading(false);
    }
  ...
  async function save(e?: FormEvent) {
    e?.preventDefault();
    if (!companyId) return;
    if (!form.code.trim() || form.code.startsWith("__DRAFT__")) {
      setError("El código de artículo es obligatorio.");
      return;
    }
    if (
      !form.commercial_description?.trim() &&
      !form.technical_description?.trim()
    ) {
      setError("Introduce al menos una descripción del artículo.");
      return;
    }
    setSaving(true);
    setError("");
    try {
      if (productId === null) {
        await createProduct(companyId, { ...form, code: form.code.trim() });
        onSaved();
      } else if (draftMode) {
        await updateProduct(companyId, productId, {
          ...form,
          code: form.code.trim(),
          active: true,
          usage_status: "ACTIVE",
  ...
  async function mark() {
    ...
    if (
      ...
      return;
    try {
      await markProductForDeletion(companyId, productId);
      await load();
      if (draftMode) navigate("/ventas/articulos");
    } catch (e) {
      setError(
        e instanceof Error
          ? e.message
          : "No se pudo marcar el artículo para borrado.",
      );
    }
  }
  async function cancel() {
    if (draftMode && companyId && productId) {
      try {
        ...
        setError(
          e instanceof Error ? e.message : "No se pudo cancelar el borrador.",
        );
      }
    }
    navigate("/ventas/articulos");
  ...
  async function restore() {
    if (!companyId || !productId) return;
    try {
      await restoreProduct(companyId, productId);
      await load();
    } catch (e) {
      setError(
        e instanceof Error ? e.message : "No se pudo recuperar el artículo.",
      );
    }
  }
  if (loading)
    return (
      <div className="loading-block">
        {draftMode ? "Preparando nuevo artículo…" : "Cargando artículo…"}
      </div>
    );
  if (productId !== null && !product)
    return <div className="inline-error">Artículo no encontrado.</div>;
  const readOnly = (!editing || isDeleted) && !draftMode;
  return (
    <div className="module-page product-page">
      <div className="page-head">
        <div>
          <div className="eyebrow">
            VENTAS / ARTÍCULOS / {draftMode ? "NUEVO" : (productId ?? "NUEVO")}
          </div>
          <h1>
            {draftMode
              ? "Nuevo artículo"
              : productId === null
                ? "Nuevo artículo"
                : `Artículo ${product?.code || productId}`}
          </h1>
          <p>
            {draftMode
              ? "Alta del artículo; puedes configurar características y escalados antes de guardar."
              : productId === null
                ? "Alta del maestro de artículos."
      ...
      {isDeleted && !draftMode && (
        <div className="soft-delete-banner">
          Este artículo está marcado para borrado. Se conserva para mantener la
          trazabilidad y no puede utilizarse en nuevos documentos.
        </div>
      )}
      {draftMode && (
        <div className="soft-delete-banner">
          Borrador: puedes definir características, proveedores y escalados
          antes de guardar definitivamente el artículo.
        </div>
      )}
      <form
        ...
        >
          ...
          <div className="form-grid">
            ...
            <label>
              ...
              <input
                value={
                  draftMode && form.code.startsWith("__DRAFT__")
                    ? ""
                    : form.code
                }
                readOnly={readOnly}
            ...
            <label className="wide">
              ...
              <input
                ...
                readOnly={readOnly}
                onChange={(e) =>
                  update("commercial_description", e.target.value)
                }
              />
            </label>
            <label className="wide">
              ...
              <input
                ...
                readOnly={readOnly}
                onChange={(e) =>
                  update("technical_description", e.target.value)
                }
              />
            </label>
            <label>
