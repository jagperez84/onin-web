...
export function ProductCharacteristicsPanel({productId,readOnly,onError}:Props){
  ...
  function startNew(){if(readOnly)return;setEditing(0);setForm({...emptyForm});}
  ...
  async function save(){if(readOnly)return;if(!form.code.trim()||!form.description.trim()){onError('Código y descripción son obligatorios.');return}setSaving(true);try{const payload={...form,code:form.code.trim(),description:form.description.trim()};if(editing===0) await createProductCharacteristic(productId,payload);else if(editing!==null) await updateProductCharacteristic(editing,payload);cancel();await load();}catch(e){onError(e instanceof Error?e.message:'No se pudo guardar la característica.')}finally{setSaving(false)}}
