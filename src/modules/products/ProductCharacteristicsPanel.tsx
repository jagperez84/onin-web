...
export function ProductCharacteristicsPanel({
  ...
  async function load() {
    try {
      const data = await listProductCharacteristics(productId, status);
      setRows(data);
    } catch (e) {
      onError(
        e instanceof Error
  ...
  function startNew() {
    if (readOnly) return;
    setEditing(0);
    setForm({ ...emptyForm, scaled: false });
  }
  ...
  async function save() {
    ...
    if (!form.code.trim() || !form.description.trim()) {
      onError("Código y descripción son obligatorios.");
      return;
    }
    if (form.scaled && !scaled) {
      onError(
        "El artículo debe tener «Escalado» activado para crear una característica escalada.",
      );
      return;
    }
    if (form.stock_minimum < 0) {
  ...
  return (
    <section
      ...
      className="panel product-characteristics-panel product-profile-anchor"
    >
      <div className="panel-head">
        <div>
          <h2>Características / variantes</h2>
          <p>
            Variantes propias del artículo. Cada una puede tener valores
            comerciales y un stock mínimo independiente.
          </p>
        </div>
          ...
          <div className="form-grid">
            ...
            <label className="check-card">
              ...
              <span>
                ...
                <small>
                  {scaled
                    ? "La variante puede participar en el escalado del artículo."
                    : "Activa «Escalado» en el artículo para habilitarlo."}
                </small>
              </span>
            </label>
          </div>
              ...
              <tr>
                <td colSpan={10}>
                  <div className="empty-state">
                    No hay características definidas para este artículo.
                  </div>
                </td>
              </tr>
            ) : (
              rows.map((row) => {
                ...
                return (
                  <tr key={row.id}>
                    ...
                    <td>{Number(row.price_increment).toFixed(2)}</td>
                    <td>{row.stock_minimum}</td>
                    <td>{row.scaled ? "Sí" : "No"}</td>
                    <td>
                      <span
      ...
      <div className="product-characteristics-filter">
        <select
          value={status}
          onChange={(e) => setStatus(e.target.value as ProductStatus)}
          aria-label="Estado de las características"
        >
