...
type Props={productId:number;readOnly:boolean;scaled?:boolean;onSaved?:()=>void;onError:(message:string)=>void};
...
export function ProductCharacteristicsPanel({productId,readOnly,scaled=false,onSaved,onError}:Props){
  ...
  useEffect(()=>{if(readOnly){setEditing(null);setForm({...emptyForm});}},[readOnly]);
  ...
  function startNew(){if(readOnly)return;setEditing(0);setForm({...emptyForm,scaled:false});}
  function edit(row:ProductCharacteristic){if(readOnly)return;setEditing(row.id);setForm({code:row.code,description:row.description??'',upc:row.upc,ptc:row.ptc,pvp:row.pvp,price_increment:row.price_increment,stock_minimum:row.stock_minimum,active:row.active,scaled:row.scaled});}
  function cancel(){setEditing(null);setForm({...emptyForm});}
  async function save(){
    if(readOnly)return;
    if(!form.code.trim()||!form.description.trim()){onError('Código y descripción son obligatorios.');return}
    if(form.scaled&&!scaled){onError('El artículo debe tener «Escalado» activado para crear una característica escalada.');return}
    if(form.stock_minimum<0){onError('El stock mínimo no puede ser negativo.');return}
    setSaving(true);
    try{
      const payload={...form,code:form.code.trim(),description:form.description.trim()};
      if(editing===0) await createProductCharacteristic(productId,payload);
      else if(editing!==null) await updateProductCharacteristic(editing,payload);
      cancel();await load();onSaved?.();
    }catch(e){onError(e instanceof Error?e.message:'No se pudo guardar la característica.')}
    finally{setSaving(false)}
  }
  async function mark(row:ProductCharacteristic){if(readOnly)return;if(!window.confirm(`¿Marcar la característica ${row.code} para borrado? No se eliminará físicamente y podrá recuperarse.`))return;try{await markProductCharacteristicForDeletion(row.id);await load();}catch(e){onError(e instanceof Error?e.message:'No se pudo marcar para borrado.')}}
  async function restore(row:ProductCharacteristic){if(readOnly)return;try{await restoreProductCharacteristic(row.id);await load();}catch(e){onError(e instanceof Error?e.message:'No se pudo recuperar la característica.')}}

  return <section id="producto-caracteristicas" className="panel product-characteristics-panel product-profile-anchor">
    <div className="panel-head"><div><h2>Características / variantes</h2><p>Variantes propias del artículo. Cada una puede tener valores comerciales y un stock mínimo independiente.</p></div><div className="panel-head-actions">{!readOnly&&<button type="button" className="secondary-button compact" onClick={startNew}><Plus size={15}/> Añadir característica</button>}</div></div>
    {editing!==null&&!readOnly&&<div className="characteristic-inline-editor"><div className="form-grid"><label>Código *<input value={form.code} onChange={e=>setForm({...form,code:e.target.value})}/></label><label>Descripción *<input value={form.description} onChange={e=>setForm({...form,description:e.target.value})}/></label><label>UPC<input type="number" step="0.01" value={form.upc??''} onChange={e=>setForm({...form,upc:e.target.value===''?null:Number(e.target.value)})}/></label><label>PTC<input type="number" step="0.01" value={form.ptc??''} onChange={e=>setForm({...form,ptc:e.target.value===''?null:Number(e.target.value)})}/></label><label>PVP<input type="number" step="0.01" value={form.pvp??''} onChange={e=>setForm({...form,pvp:e.target.value===''?null:Number(e.target.value)})}/></label><label>Incremento de precio<input type="number" step="0.01" value={form.price_increment} onChange={e=>setForm({...form,price_increment:Number(e.target.value)})}/></label><label>Stock mínimo<input type="number" min="0" step="1" value={form.stock_minimum} onChange={e=>setForm({...form,stock_minimum:Number(e.target.value)})}/></label><label>Estado<select value={form.active?'1':'0'} onChange={e=>setForm({...form,active:e.target.value==='1'})}><option value="1">Activo</option><option value="0">Inactivo</option></select></label><label className="check-card"><input type="checkbox" checked={form.scaled} disabled={!scaled} onChange={e=>setForm({...form,scaled:e.target.checked})}/><span><strong>Escalado</strong><small>{scaled?'La variante puede participar en el escalado del artículo.':'Activa «Escalado» en el artículo para habilitarlo.'}</small></span></label></div><div className="actions"><button type="button" className="secondary-button" onClick={cancel}>Cancelar</button><button type="button" className="primary-button" disabled={saving} onClick={save}><Save size={15}/>{saving?'Guardando…':'Guardar'}</button></div></div>}
    <div className="table-panel product-table"><table><thead><tr><th>Código</th><th>Descripción</th><th>UPC</th><th>PTC</th><th>PVP</th><th>Inc. precio</th><th>Stock mín.</th><th>Escalado</th><th>Estado</th><th></th></tr></thead><tbody>{rows.length===0?<tr><td colSpan={10}><div className="empty-state">No hay características definidas para este artículo.</div></td></tr>:rows.map(row=>{const deleted=!!row.deleted_at;return <tr key={row.id}><td>{row.code}</td><td>{row.description||'—'}</td><td>{row.upc??'—'}</td><td>{row.ptc??'—'}</td><td>{row.pvp==null?'—':Number(row.pvp).toFixed(2)+' €'}</td><td>{Number(row.price_increment).toFixed(2)}</td><td>{row.stock_minimum}</td><td>{row.scaled?'Sí':'No'}</td><td><span className={`status ${deleted||!row.active?'inactive':'active'}`}>{deleted?'Marcada para borrado':row.active?'Activa':'Inactiva'}</span></td><td><div className="item-actions">{!readOnly&&!deleted&&<button className="icon-action" title="Editar" onClick={()=>edit(row)}><Edit3 size={15}/></button>}{!readOnly&&deleted?<button className="icon-action" title="Recuperar" onClick={()=>restore(row)}><Undo2 size={15}/></button>:!readOnly&&!deleted&&<button className="icon-action danger" title="Marcar para borrado" onClick={()=>mark(row)}><Trash2 size={15}/></button>}</div></td></tr>})}</tbody></table></div>
