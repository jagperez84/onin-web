...
import { Edit3, Plus, Save, Power, Search, X } from 'lucide-react';
import { getActiveCompanies } from '../../services/core/coreRepository';
import { listProductCharacteristics, type ProductCharacteristic } from '../../services/catalog/productRepository';
import { createProductScale, createProductSupplier, listProductScales, listProductSuppliers, updateProductScale, updateProductSupplier, type ProductScaleInput, type ProductScaleRow, type ProductSupplierInput, type ProductSupplierRow } from '../../services/catalog/productCommercialRepository';
...
function SupplierSearchHelp({suppliers,value,onChange}: {suppliers:SupplierRef[];value:number|null;onChange:(id:number|null)=>void}){
 ...
 return <>
   <div className="entity-lookup-field">
     <button type="button" className="entity-lookup-trigger" onClick={()=>setOpen(true)} aria-haspopup="dialog" aria-expanded={open}>
       <span className={selected?'':'entity-lookup-placeholder'}>{selected?.name??'Seleccionar proveedor…'}</span>
       <Search size={16}/>
     </button>
     {selected&&<button type="button" className="entity-lookup-clear" title="Quitar proveedor" onClick={()=>onChange(null)}><X size={14}/></button>}
   </div>
   {open&&<div className="entity-lookup-backdrop" role="presentation" onMouseDown={e=>{if(e.target===e.currentTarget)setOpen(false)}}>
     <div className="entity-lookup-dialog" role="dialog" aria-modal="true" aria-label="Buscar proveedor">
       <div className="entity-lookup-head"><div><h3>Buscar proveedor</h3><p>Busca y selecciona un proveedor de la entidad Proveedores.</p></div><button type="button" className="icon-action" title="Cerrar" onClick={()=>setOpen(false)}><X size={17}/></button></div>
       <label className="entity-lookup-search"><Search size={16}/><input autoFocus value={query} onChange={e=>setQuery(e.target.value)} placeholder="Buscar por nombre…"/></label>
       <div className="entity-lookup-results">
         {filtered.length===0?<div className="empty-state">No se han encontrado proveedores.</div>:filtered.map(s=><button key={s.id} type="button" className={`entity-lookup-result${s.id===value?' selected':''}`} onClick={()=>select(s.id)}><span>{s.name}</span>{s.id===value&&<span className="status active">Seleccionado</span>}</button>)}
       </div>
...
export function ProductCommercialPanel({productId,onError}:Props){
 ...
 async function saveSupplier(){if(!supplierForm.supplier_party_id){onError('Selecciona un proveedor.');return}setSaving(true);try{if(supplierEditing===0)await createProductSupplier(productId,supplierForm as ProductSupplierInput);else if(supplierEditing!==null)await updateProductSupplier(supplierEditing,supplierForm as Partial<ProductSupplierInput>);cancelSupplier();await load();}catch(e){onError(e instanceof Error?e.message:'No se pudo guardar el proveedor.')}finally{setSaving(false)}}
