...
export type QuotationLineConfiguratorProps = {
  isOpen: boolean;
  onClose: () => void;
  onConfirm: (snapshot: QuotationLineSnapshot, lineDraft: QuotationLineDraft) => void;
  ...
  productsList: Array<{ id: number; code: string; label: string; price?: number }>;
};
...
export function QuotationLineConfigurator({
  ...
  const [masterConfig, setMasterConfig] = useState<MasterProductConfiguration | null>(null);
  ...
  const [attributeDrafts, setAttributeDrafts] = useState<QuotationLineCharacteristicDraft[]>([]);
  const [dimensionValues, setDimensionValues] = useState<Record<string, number | null>>({});
  const [quantity, setQuantity] = useState<number>(initialQuantity);
  const [discountPercent, setDiscountPercent] = useState<number>(initialDiscount);
  const [customNotes, setCustomNotes] = useState<string>(initialSnapshot?.notes ?? '');

  // Snapshot and Master Comparison
  const [previewSnapshot, setPreviewSnapshot] = useState<QuotationLineSnapshot | null>(
    initialSnapshot ?? null
  );
  const [masterComparison, setMasterComparison] = useState<MasterComparisonResult>({
    hasChanged: false,
    differences: [],
  });
  ...
  }, [isOpen, initialProductId, initialSnapshot, initialQuantity, initialDiscount]);
    ...
    (async () => {
      try {
        ...
        const { data, error } = await supabase
          .from('product')
          .select('id,code,commercial_description,technical_description,sales_price')
          .eq('active', true)
          .is('deleted_at', null)
          .order('code');
        ...
        if (active && data) {
          setInternalProducts(
            data.map((p: any) => ({
              id: Number(p.id),
              code: String(p.code || ''),
              label: p.commercial_description || p.technical_description || p.code || 'Sin descripción',
  ...
  useEffect(() => {
    ...
    loadMasterProductConfiguration(selectedProductId, companyId)
      .then(config => {
        ...
        if (!initialSnapshot || initialSnapshot.article.id !== selectedProductId) {
          const initAttrs: QuotationLineCharacteristicDraft[] = config.attributes.map(a => ({
      ...
      .catch(err => {
        if (active) setLoadError(err instanceof Error ? err.message : 'Error al cargar artículo');
      })
      .finally(() => {
  ...
  const selectedProductInfo = availableProducts.find(p => p.id === selectedProductId);
  ...
  const handleConfirm = () => {
    ...
    const lineDimensions: QuotationLineDimensionDraft[] = previewSnapshot.dimensions.map(d => ({
      code: d.code,
      name: d.name,
      value: d.value,
      unit_id: d.unit_id,
      ...
      <div className="configurator-modal">
        ...
        <div className="configurator-header">
          <div className="configurator-header-info">
            <span className="configurator-eyebrow">Configurador de Línea de Presupuesto</span>
            <h2>
              {initialSnapshot
        ...
        {masterComparison.hasChanged && (
          <div className="master-change-alert">
            <div className="master-change-alert-head">
              ...
              <span>
                El artículo maestro ha sido modificado desde la creación de esta línea.
              </span>
            </div>
            <div className="master-change-diff-list">
              {masterComparison.differences.map((diff, idx) => (
                <div key={idx} className="master-change-diff-item">
                  ...
                  <span>
                    Original en presupuesto: <em>{diff.snapshotValue}</em> → Catálogo maestro:{' '}
                    <strong>{diff.masterValue}</strong>
                  </span>
                </div>
              ))}
            </div>
        ...
        <nav className="configurator-steps-nav" aria-label="Pasos de configuración">
          <button
            type="button"
          ...
          {activeStep === 1 && (
            <div className="configurator-card">
              <div className="configurator-card-title">
                ...
                <span>Selecciona el producto del catálogo para configurar la línea</span>
              </div>
              ...
              <div className="config-form-group">
                ...
                <div style={{ fontSize: '12px', color: '#64748b', marginTop: '4px' }}>
                  Mostrando {filteredProducts.length} de {availableProducts.length} artículo(s)
                </div>
              </div>
                  ...
                  <table className="config-data-table">
                    <thead>
                      <tr>
                        ...
                        <th>Descripción</th>
                        <th style={{ width: '110px', textAlign: 'right' }}>PVP Base</th>
                    ...
                    <tbody>
                      {filteredProducts.map(p => {
                        ...
                        return (
                          ...
                          >
                            <td>
                              <strong style={{ color: isSelected ? '#0284c7' : '#0f172a' }}>
                                {p.code}
                              </strong>
                            </td>
                            <td>{p.label}</td>
                  ...
                  {masterConfig && (
                    <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap', marginTop: '10px' }}>
                      ...
                      {masterConfig.product.scaled && (
                        <span className="tag-badge warning">Escalado por Medidas</span>
                      )}
                      {masterConfig.product.scaled_by_characteristic && (
                        <span className="tag-badge warning">Escalado por Variante</span>
                      )}
                      ...
                      <span className="tag-badge info">
                        {masterConfig.bomComponents.length} componente(s) despiece
                      </span>
                    </div>
                  )}
                </div>
            ...
            <>
              ...
              {activeStep === 2 && (
                <div className="configurator-card">
                  <div className="configurator-card-title">
                    ...
                    <span>Personaliza atributos y variantes del artículo</span>
                  </div>
                  ...
                  {masterConfig.characteristics.length > 0 && (
                    <div className="config-form-group">
                      <label>
                        ...
                        <span style={{ color: '#0284c7', fontSize: '11px' }}>
                          (Determina PVP, incrementos y escalado condicionado)
                        </span>
                      </label>
                      <select
                      ...
                      >
                        ...
                        {masterConfig.characteristics.map(c => (
                          <option key={c.id} value={c.id}>
                            ...
                            {c.pvp ? ` (PVP: ${formatEuro(Number(c.pvp))})` : ''}
                            {c.price_increment
                  ...
                  {masterConfig.attributes.length > 0 ? (
                    <div className="config-form-grid">
                      {masterConfig.attributes.map(attr => {
                        ...
                        return (
                          <div key={attr.assignment_id} className="config-form-group">
                            <label>
                              {attr.name || attr.code}
                              {attr.required && (
                                <span style={{ color: '#dc2626', fontSize: '11px' }}>* Obligatorio</span>
                              )}
                            </label>
                            {attr.values.length > 0 ? (
                              <select
                                value={currentDraft?.attribute_value_id ?? ''}
                                onChange={e => {
                                  const valId = e.target.value ? Number(e.target.value) : null;
                              ...
                              <label style={{ display: 'flex', alignItems: 'center', gap: '8px', cursor: 'pointer' }}>
                                <input
                                  type="checkbox"
                                  checked={currentDraft?.value_boolean === true}
                                  onChange={e => {
                    ...
                    masterConfig.characteristics.length === 0 && (
                      <p style={{ color: '#64748b', fontSize: '13px' }}>
                        Este artículo no tiene características ni variantes configurables definidas en el maestro.
                      </p>
                    )
                  )}
                </div>
              ...
              {activeStep === 3 && (
                <div className="configurator-card">
                  ...
                  {masterConfig.dimensions.length > 0 && (
                    <div className="config-form-grid">
                      {masterConfig.dimensions.map((dim, idx) => {
                        ...
                        const stepVal = 1 / Math.pow(10, Math.max(0, dim.decimals ?? 2));
                        const unitObj = dim.unit_id ? masterConfig.unitsMap.get(dim.unit_id) : null;

                        return (
                          <div key={dim.id || idx} className="config-form-group">
                            <label>
                              {dim.name || dim.code}
                              {unitObj && (
                  ...
                  <div className="config-form-grid" style={{ marginTop: '10px' }}>
                    <div className="config-form-group">
                      <label>Cantidad de la línea ({masterConfig.baseUnit?.code || 'ud'})</label>
                      <input
                        type="number"
                        min="0.01"
                        step="1"
              ...
              {activeStep === 4 && previewSnapshot && (
                <div className="pricing-breakdown-card">
                  <div className="configurator-card-title">
                    ...
                    <span>Calculado automáticamente por el motor de precios</span>
                  </div>

                  <table className="pricing-steps-table">
                    <tbody>
                      {previewSnapshot.pricing.explainable_steps.map((step, idx) => (
                        <tr
                          key={idx}
              ...
              {activeStep === 5 && previewSnapshot && (
                <div className="configurator-card">
                  <div className="configurator-card-title">
                    ...
                    <span>Evaluación técnica de componentes, patrones de corte y disponibilidad</span>
                  </div>
                  ...
                  <div className="sub-tabs-bar">
                    ...
                    >
                      ...
                      Despiece ({previewSnapshot.breakdown.components.length})
                    </button>
                    <button
                      type="button"
                    ...
                    >
                      ...
                      Cortes y Mermas (
                      {previewSnapshot.cuts.canvas_cuts.length + previewSnapshot.cuts.profile_cuts.length}
                      )
                    </button>
                    <button
                      type="button"
                    ...
                    <div>
                      {previewSnapshot.breakdown.components.length > 0 ? (
                        <table className="config-data-table">
                          ...
                          <tbody>
                            {previewSnapshot.breakdown.components.map(comp => (
                              <tr key={comp.id}>
                                <td>
                                ...
                                <td>
                                  <strong>
                                    {comp.add_pvp ? formatEuro(comp.total_price) : 'Incluido en base'}
                                  </strong>
                                </td>
                              </tr>
                            ))}
                        ...
                        <p style={{ color: '#64748b', fontSize: '13px' }}>
                          Este artículo no tiene despiece asignado en el catálogo maestro.
                        </p>
                      )}
                    </div>
                  )}
                  ...
                  {subTab === 'cuts' && (
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                            ...
                            <tbody>
                              {previewSnapshot.cuts.canvas_cuts.map(c => (
                                <tr key={c.id}>
                                  ...
                                  <td>{c.nominal_width} m × {c.nominal_height} m</td>
                      ...
                      {previewSnapshot.cuts.profile_cuts.length > 0 && (
                        <div>
                          ...
                          <table className="config-data-table">
                            ...
                            <tbody>
                              {previewSnapshot.cuts.profile_cuts.map(p => (
                                <tr key={p.id}>
                                  <td>
                                  ...
                                  <td>{p.bars_required} ({p.standard_bar_length / 1000} m)</td>
                                  <td>{p.waste_scrap_total} mm</td>
                                  <td>
                                    {p.is_reusable_remainder ? (
                                      <span className="tag-badge success">Sí (≥ resto mín)</span>
                                    ) : (
                                      <span className="tag-badge warning">Descarte / Merma</span>
                                    )}
                                  </td>
                                </tr>
                              ))}
                      ...
                      {!previewSnapshot.cuts.canvas_cuts.length && !previewSnapshot.cuts.profile_cuts.length && (
                        <p style={{ color: '#64748b', fontSize: '13px' }}>
                          No hay cálculos de corte aplicables para esta configuración de producto.
                        </p>
                      )}
                    </div>
                  )}
                      ...
                      {previewSnapshot.stock_preview ? (
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                            <div>
                              <strong>Almacén:</strong> {previewSnapshot.stock_preview.warehouseName}
                            </div>
                            <span
                              className={`tag-badge ${
                                previewSnapshot.stock_preview.overallStatus === 'available'
                                  ? 'success'
                                  : previewSnapshot.stock_preview.overallStatus === 'low_stock'
                                  ? 'warning'
                                  : 'danger'
                              }`}
                            >
                          ...
                          <table className="config-data-table">
                            ...
                            <tbody>
                              <tr>
                                <td>
                                  <strong>{previewSnapshot.stock_preview.mainProduct.productCode}</strong>
                                  <div style={{ fontSize: '11px', color: '#64748b' }}>
                                    {previewSnapshot.stock_preview.mainProduct.productName}
                                  </div>
                                </td>
                                <td>{previewSnapshot.stock_preview.mainProduct.requiredQuantity}</td>
                                <td>{previewSnapshot.stock_preview.mainProduct.inStock}</td>
                                <td>{previewSnapshot.stock_preview.mainProduct.reserved}</td>
                                <td><strong>{previewSnapshot.stock_preview.mainProduct.available}</strong></td>
                                <td>
                                  <span
                                    className={`tag-badge ${
                                      previewSnapshot.stock_preview.mainProduct.hasSufficientStock
                                        ? 'success'
                                        : 'danger'
                                    }`}
                                  >
                              ...
                              {previewSnapshot.stock_preview.componentsStock.map(comp => (
                                <tr key={comp.productId}>
                                  <td>
                                    <strong>{comp.productCode}</strong>
                                    <div style={{ fontSize: '11px', color: '#64748b' }}>{comp.productName}</div>
                                  </td>
                                  <td>{comp.requiredQuantity}</td>
                                  <td>{comp.inStock}</td>
                                  <td>{comp.reserved}</td>
                                  <td><strong>{comp.available}</strong></td>
                                  <td>
                                    <span
                                      className={`tag-badge ${
                                        comp.hasSufficientStock ? 'success' : 'danger'
                                      }`}
                                    >
                                      {comp.hasSufficientStock ? 'Disponible' : 'Falta Stock'}
                                    </span>
                                  </td>
                                </tr>
                              ))}
                        ...
                        <p style={{ color: '#64748b', fontSize: '13px' }}>
                          El artículo no requiere seguimiento de stock o no hay almacén asignado.
                        </p>
                      )}
                    </div>
                  )}
              ...
              {activeStep === 6 && previewSnapshot && (
                <div className="configurator-card">
                  <div className="configurator-card-title">
                    ...
                    <span>Verifica todos los datos antes de confirmar la línea</span>
                  </div>

                  <div className="config-form-grid">
                    <div>
                      <strong>Artículo:</strong>
                      <div>{previewSnapshot.article.code} · {previewSnapshot.article.commercial_description || previewSnapshot.article.technical_description}</div>
                    </div>
                    {previewSnapshot.selected_variant && (
                      <div>
                        <strong>Variante:</strong>
                        <div>{previewSnapshot.selected_variant.code} · {previewSnapshot.selected_variant.description}</div>
                      </div>
                    )}
                    <div>
                      <strong>Dimensiones:</strong>
                      <div>
                        {previewSnapshot.dimensions.map(d => `${d.name}: ${d.value ?? 0} ${d.unit_code}`).join(' × ') || 'Sin dimensiones'}
                      </div>
                    </div>
                    <div>
                      <strong>Características seleccionadas:</strong>
                      <div>
                        {previewSnapshot.selected_attributes.map(a => `${a.name}: ${a.value_label}`).join(', ') || 'Sin características'}
                      </div>
                    </div>
                  </div>

                  <div className="config-form-group" style={{ marginTop: '10px' }}>
                  ...
                  <div style={{ background: '#f8fafc', padding: '14px', borderRadius: '8px', border: '1px solid #e2e8f0', marginTop: '10px' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '6px' }}>
                      ...
                      <strong>{formatEuro(previewSnapshot.pricing.unit_price)}</strong>
                    </div>
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '6px' }}>
                      <span>Cantidad ({previewSnapshot.article.base_unit_code}):</span>
                      <strong>{previewSnapshot.quantity}</strong>
                    </div>
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '6px' }}>
                      <span>Descuento aplicado:</span>
                      <strong>{previewSnapshot.pricing.discount_percent}% (-{formatEuro(previewSnapshot.pricing.discount_amount * previewSnapshot.quantity)})</strong>
                    </div>
                    <div style={{ display: 'flex', justifyContent: 'space-between', borderTop: '1px solid #cbd5e1', paddingTop: '8px', fontSize: '15px' }}>
                      ...
                      <strong style={{ color: '#0f172a' }}>{formatEuro(previewSnapshot.pricing.net_amount)}</strong>
                    </div>
                  </div>
                </div>
              )}
