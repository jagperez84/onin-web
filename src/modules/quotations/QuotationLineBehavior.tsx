...
const normalBehavior:ProductLineBehavior={
  id:0,company_id:0,code:'NORMAL',name:'Normal',description:null,
  quantity_enabled:true,price_enabled:true,discount_enabled:true,
...
export function effectiveLineBehavior(behavior:ProductLineBehavior|null|undefined){
...
export function QuotationLineBehavior({behavior}:Props){
  ...
  return <div className="quotation-line-behavior" aria-label={`Comportamiento: ${current.name}`}>
    <span className="quotation-line-behavior-name">{current.name}</span>
    <span className="quotation-line-behavior-capabilities">{advanced.join(' · ')}</span>
  </div>;
}
