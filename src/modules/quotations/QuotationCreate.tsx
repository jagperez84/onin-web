  ...
  <form id="quotation-create-form" className="detail-grid" onSubmit={save}>
   <section className="panel quotation-header-panel">
    ...
    {!headerCollapsed&&<div className="form-grid">
     ...
     <label className="address-field">Dirección de facturación<select value={billingId??''} onChange={e=>setBillingId(e.target.value?Number(e.target.value):null)}><option value="">Sin especificar</option>{addresses.map(a=><option key={a.id} value={a.id}>{addressLabel(a)}</option>)}</select></label>
     <label className="address-field">Dirección de instalación<select value={installationId??''} onChange={e=>setInstallationId(e.target.value?Number(e.target.value):null)}><option value="">Sin especificar</option>{addresses.map(a=><option key={a.id} value={a.id}>{addressLabel(a)}</option>)}</select></label>
    </div>}
   </section>
