...
export function QuotationCreate(){
 ...
 useEffect(()=>{
   if(!customerId){setAddresses([]);setBillingId(null);setInstallationId(null);return;}
   void customerAddresses(customerId).then(rows=>{
     setAddresses(rows);
   ...
   }).catch(e=>setError(e instanceof Error?e.message:'No se pudieron cargar las direcciones del cliente.'));
 },[customerId]);
 ...
 const product=(i:number,value:string)=>{const id=value?Number(value):null;const p=opts?.products.find((x:any)=>x.id===id);setLines(xs=>xs.map((l,j)=>j===i?{...l,product_id:id,description:p?.label||l.description,unit_price:p?.price??l.unit_price}:l));};
 ...
 return <div className="module-page">
   <div className="page-head">
     <div><div className="eyebrow">VENTAS / PRESUPUESTOS / NUEVO</div><h1>Nuevo presupuesto</h1><p>Crear presupuesto en estado borrador.</p></div>
   ...
   <form id="quotation-create-form" className="detail-grid" onSubmit={save}>
     <section className="panel">
       <div className="panel-head"><div><h2>Datos generales</h2><p>Cliente, condiciones comerciales y direcciones del presupuesto.</p></div></div>
       <div className="form-grid">
         ...
         <label>Fecha<input type="date" value={issueDate} onChange={e=>setIssueDate(e.target.value)} required/></label>
         <label>Válido hasta<input type="date" value={validUntil} onChange={e=>setValidUntil(e.target.value)}/></label>
     ...
     <section className="panel">
       <div className="panel-head"><div><h2>Observaciones</h2><p>Información adicional que quedará asociada al presupuesto.</p></div></div>
       <label className="wide-field">Observaciones<textarea rows={5} value={notes} onChange={e=>setNotes(e.target.value)} /></label>
     </section>
   </form>
...
function LookupSelect({label,required=false,compact=false,options,value,onChange,placeholder}:{label?:string;required?:boolean;compact?:boolean;options:Option[];value:number|null;onChange:(id:number|null)=>void;placeholder:string}){
 ...
 const filtered=useMemo(()=>{const q=query.trim().toLocaleLowerCase();if(!q)return options.slice(0,8);return options.filter(x=>`${x.code??''} ${x.label}`.toLocaleLowerCase().includes(q)).slice(0,8);},[options,query]);
 return <div className={`lookup-field ${compact?'lookup-field-compact':''}`}>
   ...
   <div className="lookup-control">
     <Search size={15}/><input required={required&&!value} value={open?query:(selected?.label??'')} placeholder={placeholder} onFocus={()=>{setOpen(true);if(selected)setQuery(selected.label);}} onChange={e=>{setQuery(e.target.value);setOpen(true);if(value!==null)onChange(null);}} />
     {selected&&<button type="button" className="lookup-clear" aria-label="Limpiar selección" onClick={()=>{setQuery('');onChange(null);setOpen(false);}}><X size={14}/></button>}
   </div>
   {open&&<>
     <button type="button" className="lookup-dismiss" aria-label="Cerrar resultados" onClick={()=>setOpen(false)} />
     <div className="lookup-results">
       {filtered.length===0?<small>No se han encontrado resultados.</small>:filtered.map(x=><button type="button" key={x.id} onClick={()=>{onChange(x.id);setQuery('');setOpen(false);}}><strong>{x.code?`${x.code} · `:''}{x.label}</strong></button>)}
     </div>
   </>}
 </div>;
}
