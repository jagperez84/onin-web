...
import { calculateQuotationLineByProductId, type QuotationLineSnapshot } from '../../services/sales/quotationLineCalculationService';
...
const blank = (): Line => ({ product_id: null, description: '', quantity: 1, unit_price: 0, discount_percent: 0, line_behavior: null, product_definition_snapshot: null, dimensions: [], characteristics: [], specific_data: {}, configuration_snapshot: null });
type Option = { id: number; label: string; code?: string; price?: number; lineBehavior?: ProductLineBehavior | null };

function clone<T>(value: T): T { return JSON.parse(JSON.stringify(value)); }
function dimensionsFromDefinition(definition: ProductLineDefinition) { return definition.dimensions.map((d, i) => ({ code: d.code, name: d.name, value: null, unit_id: d.unit_id, sort_order: i })); }
function characteristicsFromDefinition(definition: ProductLineDefinition) { return definition.characteristics.map(c => ({ attribute_id: c.attribute_id, attribute_value_id: null, value_text: null, value_number: null, value_boolean: null })); }
...
export function QuotationCreate() {
  ...
  const [billingAddress, setBillingAddress] = useState<AddressDraft>(emptyAddress());
  const [installationAddress, setInstallationAddress] = useState<AddressDraft>(emptyAddress());
  ...
  useEffect(() => {
    void quotationOptions().then(setOpts).catch(e => setError(e instanceof Error ? e.message : 'No se pudieron cargar los datos.')).finally(() => setLoading(false));
  }, []);
  ...
  const totals = useMemo(() => lines.reduce((a, l) => {
    ...
    return { discount: a.discount + discount, net: a.net + net, tax: a.tax + tax, total: a.total + net + tax };
  }, { discount: 0, net: 0, tax: 0, total: 0 }), [lines, taxPercent]);

  const updateLine = (i: number, patch: Partial<Line>) => setLines(xs => xs.map((l, j) => j === i ? { ...l, ...patch } : l));

  const recalculateLine = async (i: number, productId: number | null, dimensions: QuotationLineDimensionDraft[], silent = true) => {
    if (productId === null) return;
    const request = (calculationRequests.current[i] ?? 0) + 1;
    calculationRequests.current[i] = request;
    try {
      const result = await calculateQuotationLineByProductId({ productId, dimensions });
      if (calculationRequests.current[i] === request) updateLine(i, { unit_price: result.unit_price });
    } catch (e) {
      if (!silent && calculationRequests.current[i] === request) setError(e instanceof Error ? e.message : 'No se pudo recalcular el precio del artículo.');
    }
  };

  const selectProduct = async (i: number, id: number | null) => {
    if (id === null) {
      calculationRequests.current[i] = (calculationRequests.current[i] ?? 0) + 1;
      updateLine(i, blank());
      setExpandedLines(s => { const n = new Set(s); n.delete(i); return n; });
      return;
    }
    ...
    try {
      ...
      const discount = customerId ? await customerProductDiscount(customerId, id) : null;
      const line: Line = {
        product_id: id,
      ...
      updateLine(i, line);
      setExpandedLines(s => new Set(s).add(i));
      void recalculateLine(i, id, line.dimensions, true);
      ...
      if (hasConfig) {
        // Automatically open the guided configurator so the user can configure it immediately
        ...
        setToast(`El cliente tiene descuentos aplicados (${discount.level === 'article' ? 'artículo' : 'familia'}: ${discount.discount_percent}%).`);
      }
    } catch (e) {
  ...
  const handleConfiguratorConfirm = (snapshot: QuotationLineSnapshot) => {
    ...
    const updatedLine: Line = {
      ...
      description: snapshot.article.commercial_description || snapshot.article.technical_description || snapshot.article.code,
      quantity: snapshot.quantity,
      unit_price: snapshot.pricing.unit_price,
      discount_percent: snapshot.pricing.discount_percent,
      line_behavior: p?.lineBehavior ?? null,
      product_definition_snapshot: clone(lines[configuratorLineIndex]?.product_definition_snapshot),
    ...
    setToast(`Línea ${configuratorLineIndex + 1} configurada correctamente con snapshot.`);
  };
  ...
  const updateDimensionValue = (i: number, d: number, value: number | null) => {
    const line = lines[i];
    if (!line) return;
    const dimensions = line.dimensions.map((x, k) => k === d ? { ...x, value } : x);
    updateLine(i, { dimensions });
    if (line.product_id !== null) void recalculateLine(i, line.product_id, dimensions, false);
  };

  const updateCharacteristic = (i: number, c: number, patch: Partial<QuotationLineCharacteristicDraft>) =>
    setLines(xs => xs.map((l, j) => j === i ? { ...l, characteristics: l.characteristics.map((x, k) => k === c ? { ...x, ...patch } : x) } : l));

  const toggleLine = (i: number) => setExpandedLines(s => {
  ...
  const applyCustomerAddresses = (rows: any[]) => {
    setAddresses(rows);
    const billing = rows.find((a: any) => ['FACTURACION', 'FISCAL'].includes(String(a.address_type).toUpperCase()));
    const installation = rows.find((a: any) => String(a.address_type).toUpperCase() === 'INSTALACION');
  ...
  useEffect(() => {
    ...
    void customerAddresses(customerId).then(applyCustomerAddresses).catch(e => setError(e instanceof Error ? e.message : 'No se pudieron cargar las direcciones del cliente.'));
  }, [customerId]);
  ...
  const confirmCustomerChange = async () => {
    ...
    try {
      ...
      const rows = await customerAddresses(id);
      applyCustomerAddresses(rows);
      const discounted = await Promise.all(lines.map(async l => l.product_id ? customerProductDiscount(id, l.product_id) : null));
      setLines(xs => xs.map((l, index) => ({ ...l, discount_percent: discounted[index]?.discount_percent ?? 0 })));
      if (discounted.some(Boolean)) setToast('El cliente tiene descuentos aplicados.');
    } catch (e) {
      setError(e instanceof Error ? e.message : 'No se pudieron recalcular los datos asociados al cliente.');
    } finally {
  ...
  const save = async (e: React.FormEvent) => {
    ...
    if (lines.some(l => !hasRequiredCharacteristicValues(l))) {
      setError('Completa las características obligatorias de los artículos configurables.');
      return;
    }
    setSaving(true);
    try {
    ...
    <div className="module-page quotation-create">
      <div className="page-head">
        <div>
          <div className="eyebrow">VENTAS / PRESUPUESTOS / NUEVO</div>
          <h1>Nuevo presupuesto</h1>
          <p>Crear presupuesto en estado borrador con configuración de líneas y snapshot inmutable.</p>
        </div>
        ...
        <section className="panel quotation-header-panel">
          <div className="panel-head">
            <div>
              ...
              <p>Cliente, condiciones comerciales y direcciones del presupuesto.</p>
            </div>
            <button
              type="button"
        ...
        <section className="panel quotation-lines-panel">
          <div className="panel-head">
            <div>
              ...
              <p>
                Cada línea representa la configuración y el snapshot inmutable del artículo.
              </p>
            </div>
              ...
              <tbody>
                {lines.map((line, i) => {
                  const total =
                    ...
                    (1 + taxPercent / 100);
                  const snapshot = line.configuration_snapshot || (line.specific_data?.configuration_snapshot as QuotationLineSnapshot | undefined);
                  ...
                  return (
                    <QuotationLineRows
                      ...
                      expanded={expandedLines.has(i)}
                      opts={opts}
                      onProductChange={id => void selectProduct(i, id)}
                      onToggle={() => toggleLine(i)}
                      onOpenConfigurator={() => openLineConfigurator(i)}
                      onOpenSnapshot={() => snapshot && openSnapshotModal(snapshot, i + 1)}
                      onLinePatch={patch => updateLine(i, patch)}
                      onRemove={() => setLines(xs => xs.filter((_, j) => j !== i))}
      ...
      <ProfileSaveBar
        onSave={() => {
          const formElement = document.getElementById('quotation-create-form') as HTMLFormElement | null;
      ...
      {configuratorOpen && (
        <QuotationLineConfigurator
          ...
          initialSnapshot={currentConfiguratorLine?.configuration_snapshot ?? (currentConfiguratorLine?.specific_data?.configuration_snapshot as QuotationLineSnapshot | null) ?? null}
...
function QuotationLineRows({
  ...
  onRemove: () => void;
  canRemove: boolean;
  onDimensionChange: (line: number, dimension: number, value: number | null) => void;
  onCharacteristicChange: (line: number, characteristic: number, patch: Partial<QuotationLineCharacteristicDraft>) => void;
}) {
  const isConfigured = Boolean(snapshot);
  const isConfigurable = Boolean(
    line.line_behavior?.configuration_enabled ||
    line.line_behavior?.dimensions_enabled ||
    line.line_behavior?.characteristics_enabled ||
    line.line_behavior?.cut_calculation_enabled ||
    line.line_behavior?.canvas_cut_enabled ||
    (line.product_definition_snapshot?.dimensions && line.product_definition_snapshot.dimensions.length > 0) ||
    (line.product_definition_snapshot?.characteristics && line.product_definition_snapshot.characteristics.length > 0)
  );

  return (
    <>
      <tr className={`quotation-line-row ${isConfigured ? 'line-has-snapshot' : ''}`}>
            ...
            <div className="line-article-badges">
              {isConfigured ? (
                ...
                <button
                  ...
                  onClick={onOpenConfigurator}
                  title="Artículo configurable. Clic para configurar"
                >
        ...
        <td className="col-discount">
          <input
            ...
            onChange={e => onLinePatch({ discount_percent: Number(e.target.value) })}
          />
        </td>
        ...
        <td className="col-actions">
          <div className="line-actions-wrap">
            ...
            {line.product_id && line.product_definition_snapshot && (
              <button
                type="button"
                className={`line-btn-icon toggle ${expanded ? 'expanded' : ''}`}
                onClick={onToggle}
                title={expanded ? 'Ocultar valores rápidos' : 'Desplegar valores rápidos'}
      ...
      {expanded && line.product_id && line.product_definition_snapshot && (
        <tr className="line-details-row">
          <td colSpan={8}>
            <QuotationLineInstanceEditor
              line={line}
              lineIndex={lineIndex}
...
function QuotationLineInstanceEditor({
  line,
  lineIndex,
  onDimensionChange,
  onCharacteristicChange,
  ...
  return (
    <div className="quotation-line-editor-card">
      <div className="quotation-definition-summary">
        <div>
          <strong>Configuración del artículo</strong>
          <span>
            {definition.dimensions.length} dimensión(es) · {definition.characteristics.length} característica(s)
          </span>
        </div>
        <span className="definition-badge">Definición bloqueada</span>
      </div>
      <div className="quotation-instance-grid">
        {definition.dimensions.length > 0 && (
          <div className="quotation-edit-subsection">
            <div className="quotation-section-head">
              <div>
                <h4>Dimensiones</h4>
                <p>El nombre y la unidad vienen definidos por el artículo.</p>
              </div>
            </div>
            {line.dimensions.map((d, di) => (
              <div className="quotation-instance-row" key={`${d.code}-${di}`}>
                <div>
                  <strong>{d.name}</strong>
                  <span>{d.unit_id != null ? `Unidad ${d.unit_id}` : ''}</span>
                </div>
                <input
                  aria-label={`Valor ${d.name}`}
                  type="number"
                  step={1 / 10 ** Math.max(0, Number(definition.dimensions[di]?.decimals ?? 2))}
                  value={d.value ?? ''}
                  onChange={e =>
                    onDimensionChange(lineIndex, di, e.target.value === '' ? null : Number(e.target.value))
                  }
                />
              </div>
            ))}
          ...
          <div className="quotation-edit-subsection">
            <div className="quotation-section-head">
              <div>
                ...
                <p>Solo aparecen las características asociadas al artículo.</p>
              </div>
            </div>
            {definition.characteristics.map((c, ci) => {
              const current =
                line.characteristics[ci] ?? {
              ...
              return (
                <div className="quotation-instance-row" key={c.assignment_id}>
                  <div>
                    <strong>{c.attribute_name || c.attribute_code}</strong>
                    <span>{c.required ? 'Obligatorio' : ''}</span>
                  </div>
                  {c.values.length > 0 ? (
                    <select
        ...
        {definition.dimensions.length === 0 && definition.characteristics.length === 0 && (
          <p className="muted">Este artículo no tiene configuración.</p>
        )}
      </div>
    </div>
  );
    ...
    <fieldset className="address-editor">
      ...
      <label className="address-source">
        ...
        <select
          ...
          onChange={e => selectSource(e.target.value ? Number(e.target.value) : null)}
        >
...
function addressDraft(a: any): AddressDraft {
  return {
    ...
    label:
      a?.label ||
      ({ FACTURACION: 'Facturación', FISCAL: 'Fiscal', INSTALACION: 'Instalación' } as Record<string, string>)[
