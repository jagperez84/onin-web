...
export function QuotationDetail() {
  ...
  if (error || !data)
    return (
      <div className="module-page">
        ...
        <div className="inline-error">{error || 'Presupuesto no encontrado.'}</div>
      </div>
    );

  const totals = quotationTotals(data.lines);

  const openSnapshot = (snapshot: QuotationLineSnapshot, lineNo: number) => {
  ...
  return (
    <div className="module-page quotation-page">
      <div className="quotation-detail-head">
        <div className="quotation-detail-title">
          <div className="quotation-detail-actions">
            ...
            <Link className="primary-button" to={`/ventas/presupuestos/${data.id}/editar`}>
              <Pencil size={15} />
              Editar presupuesto
            </Link>
          </div>
          ...
          <p>
            {partyName(data.customer)} <span>·</span> {date(data.issue_date)}
          </p>
        </div>
        <div className="quotation-head-meta">
          <div>
            <span>Referencia</span>
            <strong>{data.reference || 'Sin referencia'}</strong>
          </div>
          <div>
            <span>Validez</span>
            <strong>{data.valid_until ? date(data.valid_until) : 'Sin especificar'}</strong>
          </div>
        </div>
      </div>

      <nav className="quotation-section-nav" aria-label="Navegación del presupuesto">
        <a className="active" href="#resumen">
          Resumen
        </a>
        <a href="#lineas">Líneas del presupuesto</a>
      ...
      <section id="resumen" className="quotation-info-card quotation-anchor">
        <div className="quotation-info-grid">
          <div className="quotation-info-item">
            ...
            <strong>{partyName(data.customer)}</strong>
          </div>
          <div className="quotation-info-item">
            <span>
              <UserRound size={15} />
      ...
      <section id="lineas" className="quotation-lines-section quotation-anchor">
        <div className="quotation-section-head">
          <div>
            ...
            <p>Artículos, configuración congelada (snapshot) y condiciones económicas.</p>
          </div>
          <span className="quotation-line-count">
            {data.lines?.length || 0} {(data.lines?.length || 0) === 1 ? 'línea' : 'líneas'}
          </span>
        </div>
            ...
            <table>
              ...
              <tbody>
                {(data.lines || [])
                  ...
                  .map(l => {
                    const snapshot = l.specific_data?.configuration_snapshot as QuotationLineSnapshot | undefined;

                    return (
                      <tr key={l.id}>
                        <td>{l.line_no}</td>
                        <td>
                          <strong className="quotation-product-code">{l.product?.code || 'Manual'}</strong>
                        </td>
                        <td>
                          <div>{l.description || l.product?.commercial_description || '—'}</div>
                          {snapshot && (
                            <div style={{ fontSize: '11px', color: '#64748b', marginTop: '2px' }}>
                              {snapshot.dimensions.map(d => `${d.name}: ${d.value ?? 0} ${d.unit_code}`).join(' · ')}
                              {snapshot.selected_variant ? ` · ${snapshot.selected_variant.code}` : ''}
                            </div>
                          )}
                        </td>
                        <td>{Number(l.quantity).toLocaleString('es-ES')}</td>
                        <td>{money(Number(l.unit_price || 0))}</td>
                        <td>{Number(l.discount_percent || 0)}%</td>
                        <td>{Number(l.tax_percent || 0)}%</td>
                        ...
                        <td>
                          {snapshot && (
                            <button
                              ...
                              style={{ background: '#f0fdf4', color: '#15803d', whiteSpace: 'nowrap' }}
      ...
      {data.notes && (
        <section id="observaciones" className="panel quotation-notes quotation-anchor">
          <div className="quotation-section-head">
            <div>
              <h2>Observaciones</h2>
              <p>Información adicional del presupuesto.</p>
            </div>
          </div>
          <div className="quotation-note-body">
            <FileText size={17} />
            <p>{data.notes}</p>
          </div>
        </section>
      ...
      <div className="quotation-foot-meta">
        <div>
          <span>
            <Hash size={13} />
            Identificador
