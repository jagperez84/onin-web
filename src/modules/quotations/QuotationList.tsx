...
export function QuotationList() {
  ...
  const counts = useMemo(() => {
    return {
      ...
      accepted: enrichedRows.filter((r) => r.effectiveStatus === "ACCEPTED").length,
      rejected: enrichedRows.filter((r) => r.effectiveStatus === "REJECTED").length,
      expired: enrichedRows.filter((r) => r.effectiveStatus === "EXPIRED").length,
    };
  }, [enrichedRows]);
    ...
    <div className="module-page quotation-page">
      ...
      <div className="quotation-list-summary">
        <div className="quotation-summary-card">
          <div>
            ...
            <small>{counts.draft} borradores · {counts.sent} enviados · {counts.accepted} aceptados</small>
          </div>
        </div>
        ...
        <div className="quotation-summary-card">
          <div>
            ...
            <strong className={counts.expired > 0 ? "expired-text" : ""}>{counts.expired}</strong>
            <small>{counts.expired > 0 ? "Requieren renovar fecha de validez" : "Ningún presupuesto vencido"}</small>
          </div>
        </div>
      </div>
      ...
      <div className="toolbar">
        <div className="search-box">
          <Search size={17} />
          <input autoFocus value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Buscar por número, código o referencia…" aria-label="Buscar presupuesto" />
        </div>
        ...
        <table>
          <thead>
            <tr>
              ...
              <th className="numeric" style={{ width: "120px" }}>Importe</th>
              <th style={{ width: "100px" }}></th>
            </tr>
          </thead>
          <tbody>
            {loading ? (
              <tr><td colSpan={9} style={{ textAlign: "center", padding: "32px" }}>Cargando presupuestos…</td></tr>
                ...
                <tr key={r.id} className="clickable-row">
                  <td>
                    <div className="quotation-number">
                      <Link className="primary-link" to={`/ventas/presupuestos/${r.id}`}>{r.code}</Link>
                      <small>Nº {r.id}</small>
                    </div>
                  </td>
                  <td>{new Date(`${r.issue_date}T00:00:00`).toLocaleDateString("es-ES")}</td>
                  <td><span className={r.effectiveStatus === "EXPIRED" ? "expired-text" : ""}>{r.valid_until ? new Date(`${r.valid_until}T00:00:00`).toLocaleDateString("es-ES") : "—"}</span></td>
                  <td>
                    <div className="quotation-customer">
                      <strong>{customerName(r)}</strong>
                      {customerLegalName(r) && <span>{customerLegalName(r)}</span>}
                    </div>
                  </td>
                  <td>{r.reference || "—"}</td>
                  <td>{commercialName(r)}</td>
                  <td><span className={`quotation-status ${r.effectiveStatus.toLowerCase()}`}>{statusLabel(r.effectiveStatus)}</span></td>
                  <td className="numeric"><strong>{money(Number(r.total_amount || 0))}</strong></td>
                  <td>
                    <div className="row-quick-actions">
                      <Link className="secondary-button compact" to={`/ventas/presupuestos/${r.id}`} title="Ver detalle del presupuesto"><Eye size={13} /></Link>
                      {r.effectiveStatus === "EXPIRED" && (
                        <button type="button" className="primary-button compact renew-quick-btn" title="Renovar fecha de validez" onClick={() => setRenewQuote(r)}><RefreshCw size={13} /></button>
                      )}
                      {r.effectiveStatus === "DRAFT" && (
                        <button type="button" className="secondary-button compact" title="Enviar por email" onClick={() => setEmailQuote(r)}><Mail size={13} /></button>
                      )}
                    </div>
                  </td>
                </tr>
      ...
      {emailQuote && (
        <QuotationEmailModal isOpen={!!emailQuote} onClose={() => setEmailQuote(null)} quotation={emailQuote} onSentSuccess={() => { setToast("Presupuesto enviado por correo. Estado actualizado a Enviado."); void load(); }} />
      )}

      {renewQuote && (
        <QuotationRenewModal isOpen={!!renewQuote} onClose={() => setRenewQuote(null)} quotation={renewQuote} onRenewSuccess={() => { setToast("Fecha de validez renovada con éxito. Presupuesto en estado Enviado."); void load(); }} />
      )}
    </div>
  );
}
