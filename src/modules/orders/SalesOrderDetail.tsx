...
export function SalesOrderDetail(){
 ...
 useEffect(()=>{let active=true;(async()=>{try{const row=await getSalesOrder(Number(id));if(!active)return;if(!row){setData(null);setError('Pedido no encontrado.');return;}setData(row);const lines=row.lines||[];const sheets=await Promise.all(lines.map(async(line:any)=>[Number(line.id),await getWorkSheetBySalesOrderLine(Number(line.id))] as const));if(active)setCutSheets(Object.fromEntries(sheets.filter(([,sheet])=>sheet).map(([lineId,sheet])=>[lineId,sheet as WorkSheet])))}catch(e){if(active)setError(e instanceof CoreRepositoryError?e.message:'No se pudo cargar el pedido.')}finally{if(active)setLoading(false)}})();return()=>{active=false}},[id]);
 if(loading)return <div className="loading-block">Cargando pedido…</div>;
 if(error||!data)return <div className="module-page"><div className="inline-error">{error||'Pedido no encontrado.'}</div></div>;
 ...
 return <div className="module-page sales-order-detail-page">
  ...
  {cutLine&&<ProfileCutModal line={cutLine} companyId={companyId} salesOrderId={data.id} reference={`${data.code} · Línea ${cutLine.line_no}`} onClose={async()=>{setCutLine(null);const sheet=await getWorkSheetBySalesOrderLine(Number(cutLine.id)).catch(()=>null);if(sheet)setCutSheets(current=>({...current,[Number(cutLine.id)]:sheet}));}}/>}<QuotationLineSnapshotModal isOpen={Boolean(otdSnapshot)} onClose={()=>setOtdSnapshot(null)} snapshot={otdSnapshot} lineNo={otdLineNo}/>
 </div>;
}
