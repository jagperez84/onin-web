...
type Props = {
  calculation: LonaCutCalculationResult | null;
  type: LonaCutType;
  stockDimensions: number[];
  stockUnits: string[];
  cutLine: number;
  cutOutput: number;
  unit: string | null;
...
export function LonaCutDiagram({ calculation, type, stockDimensions, stockUnits, cutLine, cutOutput, unit }: Props) {
  ...
  const stockLength = stockDimensions[0];
  const stockWidth = stockDimensions[1];
  ...
  if (type === 'Degradee') {
    segments.push({ kind: 'PANEL', length: Math.min(cutLine, stockLength), label: 'Pieza degradé' });
  } else if (type === 'Retal Maxi' || type === 'Retal Mini') {
    const remainderCount = calculation.pieces.filter(piece => piece.kind === 'REMAINDER').length;
    const panelLength = selectedWidth;
    if (remainderCount >= 1 && remainder > 0) segments.push({ kind: 'REMAINDER', length: remainder, label: 'Retal izquierdo' });
    for (let index = 0; index < calculation.fullPanels; index += 1) segments.push({ kind: 'PANEL', length: panelLength, label: `Paño ${index + 1}` });
  ...
  return <div className="lona-cut-diagram">
    <div className="lona-diagram-stage">
      <div className="lona-roll" aria-label={`Material ${dimension(stockLength, stockUnits[0] || unit)} × ${dimension(stockWidth, stockUnits[1] || unit)}`}>
        {segments.map((segment, index) => <div key={`${segment.kind}-${index}`} className={`lona-roll-piece ${segment.kind.toLowerCase()}`} style={{ width: `${(segment.length / total) * 100}%` }} title={`${segment.label}: ${dimension(segment.length, unit)}`}>
          <div className="lona-roll-piece-content"><strong>{segment.label}</strong><small>{dimension(segment.length, unit)} × {dimension(Math.min(cutOutput, stockWidth), stockUnits[1] || unit)}</small></div>
        </div>)}
        <span className="lona-roll-dimension">{dimension(stockLength, stockUnits[0] || unit)}</span>
        <span className="lona-roll-width">{dimension(stockWidth, stockUnits[1] || unit)}</span>
      </div>
    </div>
    <div className="lona-diagram-legend"><span><i className="lona-legend-box panel"/>Paño</span><span><i className="lona-legend-box remainder"/>Resto</span><span><i className="lona-legend-box unused"/>Margen</span></div>
    <div className="lona-diagram-caption"><Ruler size={12}/> Material {dimension(stockLength, stockUnits[0] || unit)} × {dimension(stockWidth, stockUnits[1] || unit)} · Corte {dimension(cutLine, unit)} × {dimension(cutOutput, unit)}</div>
  </div>;
}
