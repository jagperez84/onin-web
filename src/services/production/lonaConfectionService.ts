...
export type LonaConfectionComponent = { index:number; productId:number; productCode:string; productName:string; characteristicId:number|null; characteristicCode:string|null; characteristicName:string|null; colorId:number|null; colorCode:string|null; colorName:string|null; colorHex:string|null; quantity:number; line:number|null; output:number|null; lineUnit:string|null; outputUnit:string|null; lineDimensionCode:string|null; outputDimensionCode:string|null; sourceComponent:OtdComponentSnapshot; productConfiguration:MasterProductConfiguration; };
export type LonaCutGeometry = { width:number; height:number; widthLabel:string; heightLabel:string };
export type LonaStockCandidate = { stockItemId:number; warehouseId:number; warehouseCode:string; warehouseName:string; productId:number; characteristicId:number|null; characteristicCode:string|null; colorId:number|null; colorCode:string|null; quantity:number; sourceDimensions:number[]; sourceDimensionUnits:string[]; cutDimensions:[number,number]; cutDimensionUnits:string[]; remainderDimensions:[number,number]|null; remainderDimensionUnits:string[]; rotated:boolean; score:number; reason:string; };
...
export type LonaConfectionWorkSheet = { id:number; code:string; issueDate:string; status:string; orderLineId:number; productCode:string|null; productName:string|null; characteristicName:string|null; colorCode:string|null; colorName:string|null; colorHex:string|null; quantity:number; requiredDimensions:number[]; requiredDimensionUnits:string[]; unitSymbol:string|null; selectionMode:string|null; selectionReason:string|null; createdBy:string|null; lines:Array<{id:number;lineNo:number;warehouseCode:string|null;warehouseName:string|null;stockItemId:number|null;sourceDimensions:number[];sourceDimensionUnits:string[];cutDimensions:number[];cutDimensionUnits:string[];quantity:number;remainderDimensions:number[];remainderDimensionUnits:string[]}>; };
...
type ColorRow={id:number;code:string|null;name:string|null};
type WarehouseRow={id:number;code:string|null;name:string|null};
type StockItemRow={id:number;product_id:number;warehouse_stock_id:number;characteristic_id:number|null;color_id:number|null;quantity:number;dimension_values:unknown;dimension_units:unknown;status:string};
 ...
 */
export async function allocateLonaStockForPieces(input:{companyId:number;productId:number;characteristicId?:number|null;characteristicCode?:string|null;colorId?:number|null;colorCode?:string|null;pieces:LonaPieceRequirement[];unit?:string|null}):Promise<LonaPieceAllocation[]>{
 const c=client();
 if(!input.pieces.length)return[];
 const {data:itemData,error:itemError}=await c.from('warehouse_stock_item').select('id,product_id,warehouse_stock_id,characteristic_id,color_id,quantity,dimension_values,dimension_units,status').eq('product_id',input.productId).gt('quantity',0).eq('status','AVAILABLE').order('created_at',{ascending:true}).limit(500);
 ...
 const colorIds=[...new Set(items.map(row=>row.color_id==null?null:Number(row.color_id)).filter((id):id is number=>id!=null))];
 const [stocksResult,characteristicsResult,colorsResult]=await Promise.all([stockIds.length?c.from('warehouse_stock').select('id,warehouse_id').in('id',stockIds):Promise.resolve({data:[],error:null}),characteristicIds.length?c.from('product_attribute').select('id,code,description:name').in('id',characteristicIds):Promise.resolve({data:[],error:null}),colorIds.length?c.from('color').select('id,code,name').in('id',colorIds):Promise.resolve({data:[],error:null})]);
 if(stocksResult.error)throw new CoreRepositoryError(stocksResult.error.message);if(characteristicsResult.error)throw new CoreRepositoryError(characteristicsResult.error.message);if(colorsResult.error)throw new CoreRepositoryError(colorsResult.error.message);
 const stocks=(stocksResult.data??[]) as StockHeaderRow[];const characteristics=(characteristicsResult.data??[]) as CharacteristicRow[];const colors=(colorsResult.data??[]) as ColorRow[];const warehouseIds=[...new Set(stocks.map(row=>Number(row.warehouse_id)).filter(Number.isFinite))];const warehousesResult=warehouseIds.length?await c.from('warehouse').select('id,code,name').in('id',warehouseIds):{data:[],error:null};if(warehousesResult.error)throw new CoreRepositoryError(warehousesResult.error.message);
 const stockById=new Map(stocks.map(row=>[Number(row.id),row]));const characteristicById=new Map(characteristics.map(row=>[Number(row.id),row]));const colorById=new Map(colors.map(row=>[Number(row.id),row]));const warehouseById=new Map(((warehousesResult.data??[]) as WarehouseRow[]).map(row=>[Number(row.id),row]));
 ...
 const reqColorCode=(typeof input.colorCode==='string'&&input.colorCode.trim().length>0)?input.colorCode.trim().toLowerCase():null;

 const matchesCharacteristic=(row:StockItemRow):{characteristicId:number|null;characteristicCode:string|null;colorId:number|null;colorCode:string|null}|null=>{
   ...
   const colorId=row.color_id==null?null:Number(row.color_id);
   const colorObj=colorId==null?null:colorById.get(colorId);
   const colorCode=colorObj?.code??null;
   const colorCodeLower=colorObj?.code?colorObj.code.trim().toLowerCase():null;
   if(reqColorId!==null||reqColorCode!==null){
     let match=false;
     if(reqColorId!==null&&colorId!==null&&colorId===reqColorId)match=true;
     if(reqColorCode!==null&&colorCodeLower&&colorCodeLower===reqColorCode)match=true;
     if(!match)return null;
   }else if(colorId!==null)return null;

   return{characteristicId,characteristicCode,colorId,colorCode};
 };
...
export type LonaStockRollProbe = { stockItemId:number; warehouseId:number; warehouseCode:string; warehouseName:string; characteristicId:number|null; characteristicCode:string|null; colorId:number|null; colorCode:string|null; quantity:number; sourceDimensions:number[]; sourceDimensionUnits:string[]; rotated:boolean; };
 ...
 */
export async function probeLonaStockWidth(input:{companyId:number;productId:number;characteristicId?:number|null;characteristicCode?:string|null;colorId?:number|null;colorCode?:string|null}):Promise<LonaStockRollProbe|null>{
  const c=client();
  const{data:itemData,error:itemError}=await c.from('warehouse_stock_item').select('id,product_id,warehouse_stock_id,characteristic_id,color_id,quantity,dimension_values,dimension_units,status').eq('product_id',input.productId).gt('quantity',0).eq('status','AVAILABLE').limit(500);
  if(itemError)throw new CoreRepositoryError(itemError.message);
  const items=(itemData??[]) as StockItemRow[];
  ...
  for(const row of items){
    ...
    if(width>bestWidth){
      bestWidth=width;
      best={stockItemId:Number(row.id),warehouseId:Number(stock.warehouse_id),warehouseCode:warehouse.code??'—',warehouseName:warehouse.name??'—',characteristicId,characteristicCode,colorId,colorCode,quantity:Number(row.quantity||0),sourceDimensions:dimensions,sourceDimensionUnits:units,rotated:false};
    }
  }
  return best;
}
