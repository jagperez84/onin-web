...
function componentDimensions(component: any, snapshot: any) {
  ...
  const snapshotDimensions = Array.isArray(snapshot?.dimensions) ? snapshot.dimensions : [];
  return snapshotDimensions;
}

function getGeometry(component: LonaConfectionComponent): LonaCutGeometry | null {
  const list = component.sourceComponent?.dimension_list;
  const dimensions = Array.isArray(list) && list.length
    ? list
