...
export type WorkSheet = { id:number; company_id:number; code:string; document_type:'PROFILE_CUT'; issue_date:string; status:WorkSheetStatus; sales_order_id:number|null; sales_order_line_id:number|null; sales_order_line_no:number|null; sales_order_code?:string|null; product_id:number|null; product_code:string|null; product_name:string|null; characteristic_id:number|null; characteristic_code:string|null; characteristic_name:string|null; color_id:number|null; color_code:string|null; color_name:string|null; color_hex:string|null; required_length:number; quantity:number; unit_symbol?:string|null; unit_code?:string|null; reference:string|null; notes:string|null; selection_mode:string|null; selection_reason:string|null; created_by:string|null; lines:WorkSheetLine[]; };
...
export async function listWorkSheets(search='',status:WorkSheetStatus|'ALL'='ALL'):Promise<WorkSheet[]>{const c=client();let query=c.from('production_work_sheet').select('*,sales_order:sales_order_id(code),sales_order_line:sales_order_line_id(specific_data),color:color(hex)').eq('document_type','PROFILE_CUT').order('issue_date',{ascending:false}).order('id',{ascending:false});if(status!=='ALL')query=query.eq('status',status);const term=sanitizeSearchTerm(search);if(term)query=query.or(`code.ilike.%${term}%,product_code.ilike.%${term}%,product_name.ilike.%${term}%,reference.ilike.%${term}%`);const{data,error}=await query;if(error)throw new CoreRepositoryError(error.message);return(data||[]).map(row=>mapWorkSheet({...row,lines:[]}))}
export async function getWorkSheet(id:number):Promise<WorkSheet|null>{const c=client();const{data,error}=await c.from('production_work_sheet').select('*,sales_order:sales_order_id(code),sales_order_line:sales_order_line_id(specific_data),color:color(hex),lines:production_work_sheet_line(*)').eq('id',id).eq('document_type','PROFILE_CUT').maybeSingle();if(error)throw new CoreRepositoryError(error.message);return data?mapWorkSheet(data):null}
export async function getWorkSheetBySalesOrderLine(salesOrderLineId:number):Promise<WorkSheet|null>{const c=client();const{data,error}=await c.from('production_work_sheet').select('*,sales_order:sales_order_id(code),sales_order_line:sales_order_line_id(specific_data),color:color(hex),lines:production_work_sheet_line(*)').eq('sales_order_line_id',salesOrderLineId).eq('document_type','PROFILE_CUT').order('issue_date',{ascending:false}).order('id',{ascending:false}).limit(1).maybeSingle();if(error)throw new CoreRepositoryError(error.message);return data?mapWorkSheet(data):null}
export async function getWorkSheetsBySalesOrderLine(salesOrderLineId:number):Promise<WorkSheet[]>{const c=client();const{data,error}=await c.from('production_work_sheet').select('*,sales_order:sales_order_id(code),sales_order_line:sales_order_line_id(specific_data),color:color(hex),lines:production_work_sheet_line(*)').eq('sales_order_line_id',salesOrderLineId).eq('document_type','PROFILE_CUT').order('issue_date',{ascending:false}).order('id',{ascending:false});if(error)throw new CoreRepositoryError(error.message);return (data||[]).map(mapWorkSheet)}

export type WorkSheetDocumentType='PROFILE_CUT'|'LONA_CONFECTION'|'COMPONENT_CONSUMPTION';
export type AnyWorkSheetRow={id:number;code:string;document_type:WorkSheetDocumentType;issue_date:string;status:WorkSheetStatus;sales_order_id:number|null;sales_order_code:string|null;product_code:string|null;product_name:string|null;characteristic_name:string|null;color_name:string|null;quantity:number;unit_symbol:string|null};
/** Listado genérico de hojas de trabajo de cualquier tipo de documento (a diferencia de listWorkSheets, que solo cubre PROFILE_CUT). Fila reducida: para el detalle completo de una hoja de lona/componentes se usa su propio servicio, aquí solo sirve para listar/buscar. */
export async function listAllWorkSheets(search='',status:WorkSheetStatus|'ALL'='ALL',documentType:WorkSheetDocumentType='PROFILE_CUT'):Promise<AnyWorkSheetRow[]>{
  const c=client();
  let query=c.from('production_work_sheet').select('id,code,document_type,issue_date,status,sales_order_id,product_code,product_name,characteristic_name,color_name,quantity,unit_symbol,sales_order:sales_order_id(code)').eq('document_type',documentType).order('issue_date',{ascending:false}).order('id',{ascending:false});
  if(status!=='ALL')query=query.eq('status',status);
  const term=sanitizeSearchTerm(search);
  if(term)query=query.or(`code.ilike.%${term}%,product_code.ilike.%${term}%,product_name.ilike.%${term}%`);
  const{data,error}=await query;
  if(error)throw new CoreRepositoryError(error.message);
  return (data||[]).map((row:any)=>({id:Number(row.id),code:row.code,document_type:row.document_type,issue_date:row.issue_date,status:row.status,sales_order_id:row.sales_order_id==null?null:Number(row.sales_order_id),sales_order_code:row.sales_order?.code??null,product_code:row.product_code??null,product_name:row.product_name??null,characteristic_name:row.characteristic_name??null,color_name:row.color_name??null,quantity:Number(row.quantity||0),unit_symbol:row.unit_symbol??null}));
}
