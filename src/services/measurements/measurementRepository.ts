...
export type MeasurementListRow={id:number;code:string;reference:string|null;customer_id:number|null;customer_name_snapshot:string|null;site_city:string|null;site_street:string|null;site_latitude:number|null;site_longitude:number|null;zone_id:number|null;contact_date:string;measurement_date:string|null;measurement_time:string|null;assigned_mode:AssignedMode;assigned_user_id:string|null;status:MeasurementStatus};
export type MeasurementSortField='measurement_date'|'contact_date';
...
export async function listMeasurements(search='',status:'active'|'planned'|'assigned'|'in_progress'|'completed'|'quoted'|'closed'|'cancelled'|'all'='active',sortBy:MeasurementSortField='measurement_date',ascending=true,assignedToUserId?:string|null):Promise<MeasurementListRow[]>{const c=client();let q=c.from('measurement').select('id,code,reference,customer_id,customer_name_snapshot,site_city,site_street,site_latitude,site_longitude,zone_id,contact_date,measurement_date,measurement_time,assigned_mode,assigned_user_id,status').is('deleted_at',null).order(sortBy,{ascending,nullsFirst:false}).order('id',{ascending:false});if(status==='active')q=q.in('status',['PLANNED','ASSIGNED','IN_PROGRESS','COMPLETED']);else if(status!=='all')q=q.eq('status',status.toUpperCase());if(assignedToUserId)q=q.eq('assigned_user_id',assignedToUserId);const term=sanitizeSearchTerm(search);if(term)q=q.or(`code.ilike.%${term}%,reference.ilike.%${term}%,customer_name_snapshot.ilike.%${term}%,site_city.ilike.%${term}%`);const {data,error}=await q;if(error)throw new CoreRepositoryError(error.message);return(data??[])as MeasurementListRow[];}
...
export async function uploadMeasurementPhoto(measurementId:number,file:File,openingId:number|null=null):Promise<string>{const c=client();const{data:user}=await c.auth.getUser();if(!user.user)throw new CoreRepositoryError('La sesión de usuario no está disponible.');const path=`measurements/${measurementId}/${crypto.randomUUID()}.jpg`;const{error}=await c.storage.from('measurement-photos').upload(path,file,{contentType:'image/jpeg',cacheControl:'3600',upsert:false});if(error)throw new CoreRepositoryError(error.message);const{error:dbError}=await c.from('measurement_photo').insert({measurement_id:measurementId,storage_path:path,opening_id:openingId,created_by:user.user.id});if(dbError){await c.storage.from('measurement-photos').remove([path]);throw new CoreRepositoryError(dbError.message);}return path;}
export async function listMeasurementOpeningPhotos(openingId:number):Promise<MeasurementPhoto[]>{
  const c=client();
  const{data:rows,error}=await c.from('measurement_photo').select('storage_path,created_at').eq('opening_id',openingId).order('created_at',{ascending:false});
  if(error)throw new CoreRepositoryError(error.message);
  const paths=(rows??[]).map(row=>String(row.storage_path)).filter(Boolean);
  if(!paths.length)return[];
  const{data:signed,error:signError}=await c.storage.from('measurement-photos').createSignedUrls(paths,3600);
