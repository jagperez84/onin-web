...
export async function listMeasurements(search='',status:'active'|'planned'|'assigned'|'in_progress'|'completed'|'quoted'|'closed'|'cancelled'|'all'='active',sortBy:MeasurementSortField='measurement_date',ascending=true,assignedToUserId?:string|null):Promise<MeasurementListRow[]>{const c=client();let q=c.from('measurement').select('id,code,reference,customer_id,customer_name_snapshot,site_city,site_street,site_latitude,site_longitude,zone_id,contact_date,measurement_date,measurement_time,assigned_mode,assigned_user_id,status').is('deleted_at',null).order(sortBy,{ascending,nullsFirst:false}).order('id',{ascending:false});if(status==='active')q=q.in('status',['PLANNED','ASSIGNED','IN_PROGRESS']);else if(status!=='all')q=q.eq('status',status.toUpperCase());if(assignedToUserId)q=q.eq('assigned_user_id',assignedToUserId);const term=sanitizeSearchTerm(search);if(term)q=q.or(`code.ilike.%${term}%,reference.ilike.%${term}%,customer_name_snapshot.ilike.%${term}%,site_city.ilike.%${term}%`);const {data,error}=await q;if(error)throw new CoreRepositoryError(error.message);return(data??[])as MeasurementListRow[];}
