...
export async function listMeasurements(search='',status:'active'|'planned'|'assigned'|'in_progress'|'completed'|'quoted'|'closed'|'cancelled'|'all'='active'):Promise<MeasurementListRow[]>{const c=client();let q=c.from('measurement').select('id,code,reference,customer_id,customer_name_snapshot,site_city,site_street,site_latitude,site_longitude,zone_id,measurement_date,measurement_time,assigned_mode,assigned_user_id,status').is('deleted_at',null).order('measurement_date',{ascending:true,nullsFirst:false}).order('id',{ascending:false});if(status==='active')q=q.in('status',['PLANNED','ASSIGNED','IN_PROGRESS','COMPLETED']);else if(status!=='all')q=q.eq('status',status.toUpperCase());const term=sanitizeSearchTerm(search);if(term)q=q.or(`code.ilike.%${term}%,reference.ilike.%${term}%,customer_name_snapshot.ilike.%${term}%,site_city.ilike.%${term}%`);const {data,error}=await q;if(error)throw new CoreRepositoryError(error.message);return(data??[])as MeasurementListRow[];}
