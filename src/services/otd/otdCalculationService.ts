...
export type OtdCalculatedComponent = {
  ...
  dimension_list: Array<{ code: string; name: string; value: number }>;
  characteristic_id: number | null;
...
export async function loadOtdRuntimeData(otdId: number): Promise<OtdRuntimeData> {
  ...
  const selections: OtdSelection[] = (selRes.data ?? []).map((x: any) => ({
    id: Number(x.id),
    ...
    unit_id: x.unit_id == null ? null : Number(x.unit_id),
  ...
  const components: OtdComponentDef[] = rawComps.map((c: any) => {
...
export function calculateOtdRuntime(
  ...
  for (const comp of components) {
    ...
    try {
      ...
      const dimensionList: Array<{ code: string; name: string; value: number }> = [];
      ...
      for (const [dimCode, expr] of Object.entries(comp.dimension_expressions ?? {})) {
        ...
        const val = Math.max(0, dimRes.value);
        dimensions[dimCode] = val;
