    ...
    if (peek() === '(') {
      ...
      switch (upperId) {
        ...
        case 'TO_M':
          ...
          if (args.length !== 1) throw new Error(`TO_M requiere exactamente 1 argumento (ej. TO_M(ANCHO)).`);
          return args[0] / 1000;
        case 'TO_MM':
          // Converted from meters to mm
          if (args.length !== 1) throw new Error(`TO_MM requiere exactamente 1 argumento (ej. TO_MM(LARGO_M)).`);
          return args[0] * 1000;
        case 'TO_CM':
          // Converted from mm to cm
          if (args.length !== 1) throw new Error(`TO_CM requiere exactamente 1 argumento.`);
          return args[0] / 10;
        case 'TO_DM':
          // Converted from mm to dm
          if (args.length !== 1) throw new Error(`TO_DM requiere exactamente 1 argumento.`);
          return args[0] / 100;
        case 'TO_M2':
          // Converted from mm x mm to m2 (if 2 args) or mm2 to m2 (if 1 arg)
          if (args.length === 1) return args[0] / 1000000;
          if (args.length === 2) return (args[0] * args[1]) / 1000000;
          throw new Error(`TO_M2 requiere 1 o 2 argumentos (ej. TO_M2(ANCHO, ALTO)).`);
        case 'TO_CM2':
          if (args.length === 1) return args[0] / 100;
          if (args.length === 2) return (args[0] * args[1]) / 100;
          throw new Error(`TO_CM2 requiere 1 o 2 argumentos.`);
        case 'TO_MM2':
          if (args.length === 1) return args[0] * 1000000;
          if (args.length === 2) return args[0] * args[1];
          throw new Error(`TO_MM2 requiere 1 o 2 argumentos.`);
        case 'TO_KG':
          if (args.length !== 1) throw new Error(`TO_KG requiere 1 argumento (gramos a kg).`);
          return args[0] / 1000;
        case 'TO_G':
          if (args.length !== 1) throw new Error(`TO_G requiere 1 argumento (kg a gramos).`);
          return args[0] * 1000;
        default:
