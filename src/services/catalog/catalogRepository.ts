...
export type CatalogKind = 'families' | 'types' | 'units' | 'magnitudes' | 'colors' | 'attributes' | 'controlTypes' | 'mountingTypes' | 'lineBehaviors';
export type CatalogRow = {
  ...
  product_type_id?:number|null; control_type_id?:number|null; mounting_type_id?:number|null; line_behavior_id?:number|null;
...
const tableFor:Record<CatalogKind,string> = {
  ...
  controlTypes:'product_control_type',mountingTypes:'product_mounting_type',lineBehaviors:'product_line_behavior'
};
...
export async function upsertCatalog(kind:CatalogKind,companyId:number,input:{id?:number;code:string;name:string;active:boolean;description?:string|null;confectionable?:boolean;recuttable?:boolean;minimum_remainder?:number|null;product_type_id?:number|null;control_type_id?:number|null;mounting_type_id?:number|null;line_behavior_id?:number|null;quantity_enabled?:boolean;price_enabled?:boolean;discount_enabled?:boolean;dimensions_enabled?:boolean;configuration_enabled?:boolean;cut_calculation_enabled?:boolean;length_enabled?:boolean;characteristics_enabled?:boolean;canvas_cut_enabled?:boolean;data_type?:string}):Promise<void>{
 ...
 if(kind==='families') { base.confectionable=!!input.confectionable; base.recuttable=!!input.recuttable; base.minimum_remainder=input.minimum_remainder??null; base.product_type_id=input.product_type_id??null; base.control_type_id=input.control_type_id??null; base.mounting_type_id=input.mounting_type_id??null; base.line_behavior_id=input.line_behavior_id??null; }
 if(kind==='lineBehaviors') { base.description=input.description?.trim()||null; for(const key of ['quantity_enabled','price_enabled','discount_enabled','dimensions_enabled','configuration_enabled','cut_calculation_enabled','length_enabled','characteristics_enabled','canvas_cut_enabled']) base[key]=!!input[key]; }
