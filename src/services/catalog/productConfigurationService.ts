...
import type { ProductAttributeRef } from './productAttributeRepository';
...
function generateVersionHash(data: {
  ...
  const parts = [
    ...
    data.measurementType?.dimensions?.map((d: any) => `${d.code}-${d.unit_id}-${d.decimals}`).join('|') ?? '',
  ];
  return parts.join('::');
}

export async function loadMasterProductConfiguration(
  productId: number,
  companyId?: number | null
): Promise<MasterProductConfiguration> {
  const c = client();
  ...
  // Safe fetch for product_family (supports schemas with or without measurement_type_id)
  ...
  // 2. Fetch Units, Families, Line Behaviors, Characteristics, Scales, Assignments, OTD in parallel
  const [
    unitsRes,
    familiesRes,
    lineBehaviorsRes,
  ...
  const families = (familiesRes.data ?? []).map((f: any) => ({
    ...f,
    lineBehavior: f.line_behavior_id ? lineBehaviorsMap.get(f.line_behavior_id) ?? null : null,
  })) as ProductFamilyRef[];
  ...
  // 3. Load Measurement Type and Dimensions (from product or inherited from family)
  ...
  // 4. Load Attribute Values (from product or inherited from family)
  let assignments = (assignmentsRes.data ?? []) as any[];
  if (assignments.length === 0 && product.family_id) {
    const familyAttrsRes = await c
      ...
      .is('deleted_at', null)
      .order('sort_order');
    if (familyAttrsRes.data && familyAttrsRes.data.length > 0) {
      assignments = familyAttrsRes.data;
    }
  }
  const attributeIds = assignments.map(a => Number(a.attribute_id)).filter(Boolean);
  let valuesMap = new Map<number, Array<{ id: number; code: string; name: string; sort_order: number }>>();

  if (attributeIds.length > 0) {
    const { data: valuesData } = await c
  ...
  const attributes = assignments.map((a: any) => {
    ...
    return {
      ...
      values: valuesMap.get(Number(a.attribute_id)) ?? [],
    };
  });
  ...
  // 4. Load Component details for BOM
  const rawBom = (otdRes.data ?? []) as any[];
  const componentProductIds = rawBom
    .map(b => b.product_id)
    .filter((id): id is number => id != null);

  const componentProductsMap = new Map<number, Product>();
  if (componentProductIds.length > 0) {
    const { data: cpData } = await c
      .from('product')
      .select('id,code,commercial_description,technical_description,sales_price,purchase_price,base_unit_id')
      .in('id', [...new Set(componentProductIds)]);
    for (const cp of cpData ?? []) {
      componentProductsMap.set(Number(cp.id), cp as Product);
    }
  }
  ...
  const bomComponents = rawBom.map(b => {
    ...
    return {
      id: Number(b.id),
      code: String(b.code || ''),
      description: b.description || compProd?.commercial_description || compProd?.technical_description || null,
      quantity_expression: b.quantity_expression || null,
      unit_id: unit?.id ?? null,
      unit_code: unit?.code ?? null,
      unit_name: unit?.name ?? null,
      product_id: b.product_id ? Number(b.product_id) : null,
      product_code: compProd?.code ?? null,
      product_name: compProd?.commercial_description || compProd?.technical_description || null,
      unit_price: Number(compProd?.sales_price ?? 0),
      unit_cost: Number(compProd?.purchase_price ?? 0),
