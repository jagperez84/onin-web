...
import { listAttributeColors } from './attributeColorRepository';

export type ProductAttributeRef={id:number;code:string;name:string};
...
export async function listProductAttributeAssignments(productId:number):Promise<ProductAttributeAssignment[]>{
 const c=client();
 const {data,error}=await c.from('product_attribute_assignment').select('id,attribute_id,required,sort_order,active,deleted_at,scaled,pvp,product_attribute!inner(id,code,name)').eq('product_id',productId).is('deleted_at',null).order('sort_order').order('attribute_id');
 if(error)throw new CoreRepositoryError(error.message);
 return(data??[]).map((r:any)=>{const attr=attributeOf(r.product_attribute);return{assignment_id:Number(r.id),attribute_id:Number(r.attribute_id),id:Number(attr?.id??r.attribute_id),code:String(attr?.code??''),name:String(attr?.name??''),required:!!r.required,sort_order:Number(r.sort_order??0),active:!!r.active,deleted_at:r.deleted_at??null,scaled:!!r.scaled,pvp:r.pvp==null?null:Number(r.pvp)};});
}
...
export async function listProductCharacteristicConfiguration(productId:number):Promise<ProductCharacteristicConfiguration[]>{
 ...
 const [familyRes,productRes2,exclusionRes]=await Promise.all([
  familyId?c.from('product_family_attribute').select('id,attribute_id,required,sort_order,active,deleted_at,scaled,pvp,product_attribute!inner(id,code,name)').eq('family_id',familyId).eq('active',true).is('deleted_at',null).order('sort_order').order('attribute_id'):Promise.resolve({data:[],error:null} as any),
  c.from('product_attribute_assignment').select('id,attribute_id,required,sort_order,active,deleted_at,scaled,pvp,product_attribute!inner(id,code,name)').eq('product_id',productId).eq('active',true).is('deleted_at',null).order('sort_order').order('attribute_id'),
 ...
 for(const r of familyRes.data??[]){const attr=attributeOf((r as any).product_attribute);const id=Number((r as any).attribute_id);effective.set(id,{assignment_id:Number((r as any).id),attribute_id:id,product_id:productId,id:Number(attr?.id??id),code:String(attr?.code??''),name:String(attr?.name??''),required:!!(r as any).required,sort_order:Number((r as any).sort_order??0),active:!!(r as any).active,deleted_at:(r as any).deleted_at??null,source:'family',excluded:excluded.has(id),scaled:!!(r as any).scaled,pvp:(r as any).pvp==null?null:Number((r as any).pvp)});}
 for(const r of productRes2.data??[]){const attr=attributeOf((r as any).product_attribute);const id=Number((r as any).attribute_id);effective.set(id,{assignment_id:Number((r as any).id),attribute_id:id,product_id:productId,id:Number(attr?.id??id),code:String(attr?.code??''),name:String(attr?.name??''),required:!!(r as any).required,sort_order:Number((r as any).sort_order??0),active:!!(r as any).active,deleted_at:(r as any).deleted_at??null,source:'article',excluded:false,scaled:!!(r as any).scaled,pvp:(r as any).pvp==null?null:Number((r as any).pvp)});}
 return [...effective.values()].sort((a,b)=>a.sort_order-b.sort_order||a.code.localeCompare(b.code));
}

export async function listAvailableProductAttributes(companyId:number,productId:number):Promise<ProductAttributeRef[]>{const c=client();const [attrs,effective]=await Promise.all([c.from('product_attribute').select('id,code,name').eq('company_id',companyId).eq('active',true).is('deleted_at',null).order('code'),listProductCharacteristicConfiguration(productId)]);if(attrs.error)throw new CoreRepositoryError(attrs.error.message);const ids=new Set(effective.filter(x=>!x.excluded).map(x=>x.attribute_id));return((attrs.data??[])as ProductAttributeRef[]).filter(x=>!ids.has(x.id));}
 ...
 * familia o materializada): los colores de sistema de la característica menos las
 * exclusiones del nivel correspondiente (familia o artículo). Es el conjunto que se
 * ofrece para elegir al configurar una línea de presupuesto — la característica se
 * relaciona únicamente con colores, no con una lista de valores propia.
 */
export async function listEffectiveAttributeColors(attributeId:number,source:'family'|'article',assignmentId:number):Promise<EffectiveAttributeColor[]>{
  const [allColors,exclusions]=await Promise.all([
    listAttributeColors(attributeId),
    source==='family'?listFamilyAttributeColorExclusions(assignmentId):listProductAttributeColorExclusions(assignmentId),
  ]);
