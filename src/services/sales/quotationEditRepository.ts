...
async function getProductLineDefinitions(productIds: number[]): Promise<Map<number, ProductLineDefinition>> {
  const result = new Map<number, ProductLineDefinition>();
  const unique = [...new Set(productIds)];
  if (!unique.length) return result;
  const definitions = await Promise.all(unique.map(async id => [id, await getProductLineDefinition(id)] as const));
  for (const [id, definition] of definitions) result.set(id, definition);
  return result;
}
function snapshotDimensions(snapshot: ProductLineDefinition | null, rows: QuotationLineDimensionDraft[]) {
  if (!snapshot) return rows.map((x, i) => ({ code: x.code.trim(), name: x.name.trim(), value: x.value == null ? null : Number(x.value), unit_id: x.unit_id == null ? null : Number(x.unit_id), sort_order: Number.isFinite(x.sort_order) ? x.sort_order : i })).filter(x => x.code && x.name);
  const byCode = new Map(rows.map(x => [x.code.trim(), x]));
  return snapshot.dimensions.map((d, i) => {
    const draft = byCode.get(d.code);
    return { code: d.code, name: d.name, value: draft?.value == null ? null : Number(draft.value), unit_id: d.unit_id, sort_order: i };
  });
}
function snapshotCharacteristics(snapshot: ProductLineDefinition | null, rows: QuotationLineCharacteristicDraft[]) {
  const cleaned = rows.map(x => ({ attribute_id: x.attribute_id == null ? null : Number(x.attribute_id), attribute_value_id: x.attribute_value_id == null ? null : Number(x.attribute_value_id), value_text: x.value_text?.trim() || null, value_number: x.value_number == null ? null : Number(x.value_number), value_boolean: x.value_boolean ?? null })).filter(x => [x.value_text, x.value_number, x.value_boolean, x.attribute_value_id].filter(v => v !== null).length === 1);
  if (!snapshot) return cleaned;
  const allowed = new Set(snapshot.characteristics.map(c => c.attribute_id));
  return cleaned.filter(x => x.attribute_id != null && allowed.has(Number(x.attribute_id)));
}
...
export async function quotationForEdit(id: number): Promise<QuotationEditData> {
  ...
  const lines = ((q.lines ?? []) as any[]).sort((a, b) => Number(a.line_no) - Number(b.line_no)).map((line) => ({
    id: Number(line.id), line_no: Number(line.line_no), product_id: line.product_id == null ? null : Number(line.product_id), description: line.description ?? '',
    quantity: Number(line.quantity ?? 0), unit_price: Number(line.unit_price ?? 0), discount_percent: Number(line.discount_percent ?? 0),
    tax_rate_id: line.tax_rate_id == null ? null : Number(line.tax_rate_id), tax_percent: Number(line.tax_percent ?? 0),
    line_behavior_id: line.line_behavior_id == null ? null : Number(line.line_behavior_id), line_behavior_snapshot: line.line_behavior_snapshot ?? null,
  ...
  return {
    id: Number(q.id), code: q.code, customer_id: q.customer_id == null ? null : Number(q.customer_id), commercial_id: q.commercial_id == null ? null : Number(q.commercial_id), warehouse_id: q.warehouse_id == null ? null : Number(q.warehouse_id),
    ...
    payment_method_id: q.payment_method_id == null ? null : Number(q.payment_method_id), payment_term_id: q.payment_term_id == null ? null : Number(q.payment_term_id), tax_rate_id: q.tax_rate_id == null ? null : Number(q.tax_rate_id), tax_percent: Number(q.tax_percent ?? 0), issue_date: q.issue_date, valid_until: q.valid_until,
    reference: q.reference ?? '', notes: q.notes ?? '', status: q.status, lines,
  };
}

export async function updateQuotation(input: {
  id: number; customer_id: number; commercial_id: number | null; warehouse_id: number | null; billing_address_id: number | null; installation_address_id: number | null;
  billing_address: QuotationEditData['billing_address']; installation_address: QuotationEditData['installation_address']; payment_method_id: number | null; payment_term_id: number | null;
  tax_rate_id: number | null; tax_percent: number; issue_date: string; valid_until: string | null; reference: string; notes: string; lines: Array<Omit<QuotationEditLine, 'id'>>;
}) {
  const c = client();
  const cid = await companyId();
  ...
  const { data: existingLines, error: existingError } = await c.from('quotation_line').select('id,line_no,product_id,product_definition_snapshot').eq('quotation_id', input.id).order('line_no');
  ...
  const lines = input.lines.map((line, index) => {
    ...
    const snapshot = sameProduct && existing.product_definition_snapshot ? existing.product_definition_snapshot as ProductLineDefinition : line.product_id == null ? null : definitions.get(line.product_id) ?? null;
  ...
  const ba = input.billing_address; const ia = input.installation_address;
  const { error: headerError } = await c.from('quotation').update({
    customer_id: input.customer_id, commercial_id: input.commercial_id, warehouse_id: input.warehouse_id, billing_address_id: input.billing_address_id, installation_address_id: input.installation_address_id,
    ...
    payment_method_id: input.payment_method_id, payment_term_id: input.payment_term_id, tax_rate_id: input.tax_rate_id, tax_percent: taxPercent, issue_date: input.issue_date, valid_until: input.valid_until || null,
    reference: input.reference.trim() || null, notes: input.notes.trim() || null, net_amount: net, discount_amount: discount, tax_amount: tax, total_amount: net + tax, updated_at: new Date().toISOString(),
  ...
  if (oldIds.length) {
    const { error: dimensionsError } = await c.from('quotation_line_dimension').delete().in('quotation_line_id', oldIds); if (dimensionsError) throw new CoreRepositoryError(dimensionsError.message);
    const { error: characteristicsError } = await c.from('quotation_line_characteristic').delete().in('quotation_line_id', oldIds); if (characteristicsError) throw new CoreRepositoryError(characteristicsError.message);
    const { error: linesDeleteError } = await c.from('quotation_line').delete().in('id', oldIds); if (linesDeleteError) throw new CoreRepositoryError(linesDeleteError.message);
  }
  ...
  const dimensions: any[] = []; const characteristics: any[] = [];
  input.lines.forEach((line, index) => {
    const quotationLineId = lineIds.get(index + 1); if (!quotationLineId) return;
    const snapshot = lines[index].product_definition_snapshot as ProductLineDefinition | null;
    for (const d of snapshotDimensions(snapshot, line.dimensions ?? [])) dimensions.push({ ...d, quotation_line_id: quotationLineId });
    for (const ch of snapshotCharacteristics(snapshot, line.characteristics ?? [])) characteristics.push({ ...ch, quotation_line_id: quotationLineId });
  });
  if (dimensions.length) { const { error } = await c.from('quotation_line_dimension').insert(dimensions); if (error) throw new CoreRepositoryError(error.message); }
  if (characteristics.length) { const { error } = await c.from('quotation_line_characteristic').insert(characteristics); if (error) throw new CoreRepositoryError(error.message); }
  return input.id;
}
