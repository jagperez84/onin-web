...
export type ProductLineBehavior={id:number;company_id:number;code:string;name:string;description:string|null;quantity_enabled:boolean;price_enabled:boolean;discount_enabled:boolean;dimensions_enabled:boolean;configuration_enabled:boolean;characteristics_enabled:boolean};
...
export type QuotationLineCharacteristicDraft={attribute_id:number|null;attribute_value_id:number|null;value_text:string|null;value_number:number|null;value_boolean:boolean|null;color_id:number|null};
...
function definitionSnapshot(definition:ProductLineDefinition|null){return definition?{product_id:definition.product_id,measurement_type_id:definition.measurement_type_id,dimensions:definition.dimensions.map(d=>({...d})),characteristics:definition.characteristics.map(c=>({...c,colors:c.colors.map(v=>({...v}))}))}:null;}
