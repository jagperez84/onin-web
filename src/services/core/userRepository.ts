...
export async function listFieldStaffAccounts():Promise<Pick<UserAccount,'id'|'username'|'display_name'|'role_code'|'can_measure'>[]>{const c=client();const {data,error}=await c.rpc('list_field_staff');if(error)throw new CoreRepositoryError(error.message);return (data??[]) as Pick<UserAccount,'id'|'username'|'display_name'|'role_code'|'can_measure'>[];}
